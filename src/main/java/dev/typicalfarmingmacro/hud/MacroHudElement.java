...
import dev.typicalfarmingmacro.modules.farming.FastLaneSwitchManager;
 ...
 *
 * <p>Three styles controlled by {@code TfmConfig.HUD_THEME.get()}:
 * <ul>
...
public class MacroHudElement extends HudElement {
    ...
    private static final String TARGET_TITLE    = "Tfm";
    ...
    @Override public float   getX()           { return TfmConfig.HUD_X.get(); }
    @Override public float   getY()           { return TfmConfig.HUD_Y.get(); }
    @Override public void    setX(float x)    { TfmConfig.HUD_X.set((int) x); }
    @Override public void    setY(float y)    { TfmConfig.HUD_Y.set((int) y); }
    @Override public float   getScale()       { return TfmConfig.HUD_SCALE.get(); }
    @Override public void    setScale(float s){ TfmConfig.HUD_SCALE.set(s); }
    @Override public float   getWidth()       { return W; }
    @Override public float   getHeight()      { return computeHeight(); }
    @Override public boolean isVisible()      {
        net.minecraft.client.Minecraft mc = net.minecraft.client.Minecraft.getInstance();
        boolean inSupportedArea = dev.typicalfarmingmacro.util.ClientUtils.isSupportedHudArea(mc);
        return TfmConfig.SHOW_HUD.get() && (inSupportedArea || TfmConfig.SHOW_HUD_OUTSIDE_GARDEN.get());
    }
    ...
    protected void renderElement(NVGRenderer nvg, boolean editMode) {
        ...
        row(nvg, ry, String.format("BPS (%d / %.1fs)", dev.typicalfarmingmacro.util.BpsTracker.getBreakCount(), dev.typicalfarmingmacro.util.BpsTracker.getActualWindowSeconds()), String.format("%.2f", dev.typicalfarmingmacro.util.BpsTracker.getBps())); ry += ROW_H;
