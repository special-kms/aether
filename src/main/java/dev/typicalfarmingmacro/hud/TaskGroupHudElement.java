...
public class TaskGroupHudElement extends HudElement {
    ...
    public float getX() {
        float configured = switch (group) {
            case INTERMEDIARIES -> TfmConfig.INTERMEDIARIES_HUD_X.get();
            case MID_FARMING_TASKS -> TfmConfig.MID_FARMING_HUD_X.get();
            case FAILSAFES -> TfmConfig.FAILSAFES_HUD_X.get();
        };
    ...
    public float getY() {
        float configured = switch (group) {
            case INTERMEDIARIES -> TfmConfig.INTERMEDIARIES_HUD_Y.get();
            case MID_FARMING_TASKS -> TfmConfig.MID_FARMING_HUD_Y.get();
            case FAILSAFES -> TfmConfig.FAILSAFES_HUD_Y.get();
        };
        Minecraft mc = Minecraft.getInstance();
        if (mc == null || mc.getWindow() == null) {
            return configured;
    ...
    public void setX(float x) {
        if (group == Group.INTERMEDIARIES) {
            TfmConfig.INTERMEDIARIES_HUD_X.set((int) x);
        } else if (group == Group.MID_FARMING_TASKS) {
            TfmConfig.MID_FARMING_HUD_X.set((int) x);
        } else {
            TfmConfig.FAILSAFES_HUD_X.set((int) x);
        }
    }
    ...
    public void setY(float y) {
        if (group == Group.INTERMEDIARIES) {
            TfmConfig.INTERMEDIARIES_HUD_Y.set((int) y);
        } else if (group == Group.MID_FARMING_TASKS) {
            TfmConfig.MID_FARMING_HUD_Y.set((int) y);
        } else {
            TfmConfig.FAILSAFES_HUD_Y.set((int) y);
        }
    }
    ...
    public float getScale() {
        return switch (group) {
            case INTERMEDIARIES -> TfmConfig.INTERMEDIARIES_HUD_SCALE.get();
            case MID_FARMING_TASKS -> TfmConfig.MID_FARMING_HUD_SCALE.get();
            case FAILSAFES -> TfmConfig.FAILSAFES_HUD_SCALE.get();
        };
    }
    ...
    public void setScale(float s) {
        if (group == Group.INTERMEDIARIES) {
            TfmConfig.INTERMEDIARIES_HUD_SCALE.set(s);
        } else if (group == Group.MID_FARMING_TASKS) {
            TfmConfig.MID_FARMING_HUD_SCALE.set(s);
        } else {
            TfmConfig.FAILSAFES_HUD_SCALE.set(s);
        }
    }
    ...
    public boolean isVisible() {
        ...
        boolean allowedArea = inSupportedArea || TfmConfig.SHOW_HUD_OUTSIDE_GARDEN.get();
        if (!allowedArea) {
            return false;
        }
        return switch (group) {
            case INTERMEDIARIES -> TfmConfig.SHOW_INTERMEDIARIES_HUD.get();
            case MID_FARMING_TASKS -> TfmConfig.SHOW_MID_FARMING_HUD.get();
            case FAILSAFES -> TfmConfig.SHOW_FAILSAFES_HUD.get();
        };
    }
