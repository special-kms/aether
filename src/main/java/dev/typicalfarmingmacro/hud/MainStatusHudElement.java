...
public class MainStatusHudElement extends HudElement {
    ...
    @Override public float   getX()            { return TfmConfig.MAIN_STATUS_HUD_X.get(); }
    @Override public float   getY()            { return TfmConfig.MAIN_STATUS_HUD_Y.get(); }
    @Override public void    setX(float x)     { TfmConfig.MAIN_STATUS_HUD_X.set((int) x); }
    @Override public void    setY(float y)     { TfmConfig.MAIN_STATUS_HUD_Y.set((int) y); }
    @Override public float   getScale()        { return TfmConfig.MAIN_STATUS_HUD_SCALE.get(); }
    @Override public void    setScale(float s) { TfmConfig.MAIN_STATUS_HUD_SCALE.set(s); }
    @Override public float   getWidth()        { return W; }
    @Override public float   getHeight()       { return HEADER_H + CONTENT_H; }
    @Override public boolean isVisible()       { return (TfmConfig.HUD_THEME.get() & 0x1) != 0; }
    ...
    protected void renderElement(NVGRenderer nvg, boolean editMode) {
        ...
        boolean gradient  = TfmConfig.MAIN_STATUS_GRADIENT.get();
        int     gradLeft  = gradient ? TfmConfig.MAIN_STATUS_GRADIENT_LEFT.get()  : Theme.ACCENT_PRIMARY;
        int     gradRight = gradient ? TfmConfig.MAIN_STATUS_GRADIENT_RIGHT.get() : Theme.ACCENT_PRIMARY;
