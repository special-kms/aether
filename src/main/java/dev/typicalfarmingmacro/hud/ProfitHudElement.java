package dev.typicalfarmingmacro.hud;
...
import dev.typicalfarmingmacro.ui.util.Fonts;
...
public class ProfitHudElement extends HudElement {
    ...
    @Override public float getX() {
        return switch (mode) {
            case "lifetime" -> TfmConfig.LIFETIME_HUD_X.get();
            case "daily"    -> TfmConfig.DAILY_HUD_X.get();
            default         -> TfmConfig.SESSION_PROFIT_HUD_X.get();
        };
    }
    @Override public float getY() {
        return switch (mode) {
            case "lifetime" -> TfmConfig.LIFETIME_HUD_Y.get();
            case "daily"    -> TfmConfig.DAILY_HUD_Y.get();
            default         -> TfmConfig.SESSION_PROFIT_HUD_Y.get();
        };
    }
    @Override public void setX(float x) {
        switch (mode) {
            case "lifetime" -> TfmConfig.LIFETIME_HUD_X.set((int) x);
            case "daily"    -> TfmConfig.DAILY_HUD_X.set((int) x);
            default         -> TfmConfig.SESSION_PROFIT_HUD_X.set((int) x);
        }
    }
    @Override public void setY(float y) {
        switch (mode) {
            case "lifetime" -> TfmConfig.LIFETIME_HUD_Y.set((int) y);
            case "daily"    -> TfmConfig.DAILY_HUD_Y.set((int) y);
            default         -> TfmConfig.SESSION_PROFIT_HUD_Y.set((int) y);
        }
    }
    @Override public float getScale() {
        return switch (mode) {
            case "lifetime" -> TfmConfig.LIFETIME_HUD_SCALE.get();
            case "daily"    -> TfmConfig.DAILY_HUD_SCALE.get();
            default         -> TfmConfig.SESSION_PROFIT_HUD_SCALE.get();
        };
    }
    @Override public void setScale(float s) {
        switch (mode) {
            case "lifetime" -> TfmConfig.LIFETIME_HUD_SCALE.set(s);
            case "daily"    -> TfmConfig.DAILY_HUD_SCALE.set(s);
            default         -> TfmConfig.SESSION_PROFIT_HUD_SCALE.set(s);
        }
    }
    @Override public float   getWidth()  { return W; }
    @Override public float   getHeight() { return computeHeight(); }
    @Override public boolean isVisible() {
        if (!TfmConfig.PROFIT_HUD_ENABLED.get()) return false;
        return switch (mode) {
            case "lifetime" -> TfmConfig.SHOW_LIFETIME_HUD.get();
            case "daily"    -> TfmConfig.SHOW_DAILY_HUD.get();
            default         -> TfmConfig.SHOW_SESSION_PROFIT_HUD.get();
        };
    }
    @Override public String  getName()       { return title(); }
    @Override public void    savePosition()  { TfmConfig.save(); }

    // -- Height ----------------------------------------------------------------

    float computeHeight() {
        boolean sleek = TfmConfig.HUD_THEME.get() == 2;
        float h = PAD_V + TITLE_SZ + 3f;
        if (!sleek) h += 1f;   // separator
        h += 8f;               // gap after separator

        int itemCount;
        if (TfmConfig.COMPACT_PROFIT_CALCULATOR.get()) {
        ...
        if (showFarmingXp()) {
            ...
            if (TfmConfig.FARMING_HUD_XP_RATE.get()) rows++;
            if (TfmConfig.FARMING_HUD_ETA_NEXT.get()) rows++;
            if (TfmConfig.FARMING_HUD_ETA_MAX.get()) rows++;
    ...
    private boolean showFarmingXp() {
        return isSession()
                ...
                && dev.typicalfarmingmacro.modules.profit.helpers.FarmingXpTracker.hasData();
    }
    ...
    protected void renderElement(NVGRenderer nvg, boolean editMode) {
        ...
        boolean mod   = TfmConfig.HUD_THEME.get() == 1;
        boolean sleek = TfmConfig.HUD_THEME.get() == 2;
        ...
        if (showFarmingXp()) {
            ...
            boolean maxed = dev.typicalfarmingmacro.modules.profit.helpers.FarmingXpTracker.isMaxed();
            boolean paused = dev.typicalfarmingmacro.modules.profit.helpers.FarmingXpTracker.isPaused();
            int level = dev.typicalfarmingmacro.modules.profit.helpers.FarmingXpTracker.getLevel();
            float prog = dev.typicalfarmingmacro.modules.profit.helpers.FarmingXpTracker.getProgressToMax();
            ...
            if (TfmConfig.FARMING_HUD_XP_RATE.get()) {
                long perHour = dev.typicalfarmingmacro.modules.profit.helpers.FarmingXpTracker.getXpPerHour();
                String rateStr = maxed ? "MAX" : (fmt(perHour) + (paused ? " (paused)" : ""));
                row(nvg, ry, "Farming XP/hr", rateStr, 0xFF55FF55);
                ry += ROW_H;
            }

            if (TfmConfig.FARMING_HUD_ETA_NEXT.get()) {
                long etaNext = dev.typicalfarmingmacro.modules.profit.helpers.FarmingXpTracker.getEtaToNextLevelMs();
                String s = maxed ? "done" : (etaNext < 0 ? "---" : formatEta(etaNext));
                row(nvg, ry, "Next level (" + (level + 1) + ")", s, 0xFFFFAA00);
                ry += ROW_H;
            }

            if (TfmConfig.FARMING_HUD_ETA_MAX.get()) {
                long etaMax = dev.typicalfarmingmacro.modules.profit.helpers.FarmingXpTracker.getEtaToMaxMs();
