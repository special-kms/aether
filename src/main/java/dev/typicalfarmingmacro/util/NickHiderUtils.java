...
public class NickHiderUtils {
    ...
    public static String replaceNames(String text) {
        ...
        if (TfmConfig.NICK_HIDER_ENABLED.get()) {
            String replacement = TfmConfig.CUSTOM_USERNAME.get();
            if (!replacement.isEmpty()) {
                text = getUsernamePattern().matcher(text).replaceAll(Matcher.quoteReplacement(replacement));
            }
        }

        if (TfmConfig.HIDE_SERVER_ID.get()) {
            String replacement = TfmConfig.CUSTOM_SERVER_ID.get();
        ...
        if (TfmConfig.COOP_HIDER_ENABLED.get()) {
            for (String target : TfmConfig.COOP_NAMES.get()) {
        ...
        if (areSpoofValuesEnabled() && TfmConfig.CUSTOM_SB_LEVEL_ENABLED.get()) {
            Matcher m = SB_LEVEL_PATTERN.matcher(text);
            if (m.find()) {
                int level = TfmConfig.CUSTOM_SB_LEVEL.get();
    ...
    public static Component transformComponent(Component component) {
        ...
        if (areSpoofValuesEnabled()) {
            getOffsetReplacementMapped(plainText, map, PURSE_PATTERN, TfmConfig.PURSE_OFFSET.get(), replacements);
            getOffsetReplacementMapped(plainText, map, BITS_PATTERN, TfmConfig.BITS_OFFSET.get(), replacements);
            getOffsetReplacementMapped(plainText, map, COPPER_PATTERN, TfmConfig.COPPER_OFFSET.get(), replacements);
            getOffsetReplacementMapped(plainText, map, SAWDUST_PATTERN, TfmConfig.SAWDUST_OFFSET.get(), replacements);
            getOffsetReplacementMapped(plainText, map, FARMING_EXP_PATTERN, TfmConfig.FARMING_EXP_OFFSET.get(),
                    replacements);
        }
        ...
        String username = Minecraft.getInstance().getUser().getName();
        String customNick = TfmConfig.CUSTOM_USERNAME.get();

        if (TfmConfig.NICK_HIDER_ENABLED.get() && !customNick.isEmpty()) {
    ...
    private static boolean isNickHiderActive() {
        return TfmConfig.NICK_HIDER_MASTER_ENABLED.get()
    ...
    private static boolean areSpoofValuesEnabled() {
        return TfmConfig.SPOOF_VALUES_ENABLED.get();
    }
