...
public class CommandUtils {
     ...
     * This is called from TfmClient when a chat message is received.
     */
    ...
    public static boolean shouldSkipSetSpawn() {
        return TfmConfig.MACRO_DISABLE_SETSPAWN.get();
    }
    ...
    public static boolean warpGarden(Minecraft client) {
        ...
        ChatWindow window = beginChatWindow();
        dev.typicalfarmingmacro.modules.failsafe.FailsafeManager.addRotationGracePeriod(TfmConfig.FAILSAFE_ROTATION_WARP_GRACE_MS.get());
        ...
        while (System.currentTimeMillis() - startTime < MESSAGE_TIMEOUT_MS) {
            ...
            if (client.player != null) {
                ...
                if (dist > 10) {
                    dev.typicalfarmingmacro.macro.MacroState.Location loc = ClientUtils.getCurrentLocation(client);
                    if (loc == dev.typicalfarmingmacro.macro.MacroState.Location.GARDEN) {
