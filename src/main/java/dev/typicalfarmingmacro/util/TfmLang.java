...
public final class TfmLang {
    private static final Set<String> HARDCODED_TEXT = Set.of(
            ...
            "config/typicalfarmingmacro/features/local-feature.jar",
            "e.g. 1250000000",
            "e.g. 5",
            "e.g. 650000000",
    ...
    public static String keyFromText(String text) {
        if (text == null || text.isEmpty()) {
            return "text.typicalfarmingmacro.empty";
        }

        String normalized = text.toLowerCase(Locale.ROOT);
        StringBuilder builder = new StringBuilder("text.typicalfarmingmacro.");
        ...
        while (end > "text.typicalfarmingmacro.".length() && builder.charAt(end - 1) == '_') {
            end--;
        }
        if (end == "text.typicalfarmingmacro.".length()) {
            return "text.typicalfarmingmacro.empty";
        }
    ...
    private static Map<String, String> loadBundledEnglishTranslations() {
        try (var stream = TfmLang.class.getClassLoader()
                .getResourceAsStream("assets/typicalfarmingmacro/lang/en_us.json")) {
            if (stream == null) {
                return Map.of();
            }
