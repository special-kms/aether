...
import dev.typicalfarmingmacro.util.ProgrammaticAttackTracker;
...
public abstract class AbstractMacro {
    ...
    public void onEnable(Minecraft mc) {
        ...
        if (mousematRotation != null) {
            ...
            if (TfmConfig.MACRO_USE_CUSTOM_PITCH.get()) {
                float pitchVal = TfmConfig.MACRO_CUSTOM_PITCH.get();
                float hum = TfmConfig.MACRO_CUSTOM_PITCH_HUMANIZATION.get();
                if (hum > 0) {
                    pitchVal += (float) ((Math.random() - 0.5) * 2.0 * hum);
                }
                pitch = Optional.of(pitchVal);
            }
            if (TfmConfig.MACRO_USE_CUSTOM_YAW.get()) {
                float yawVal = TfmConfig.MACRO_CUSTOM_YAW.get();
                float hum = TfmConfig.MACRO_CUSTOM_YAW_HUMANIZATION.get();
                if (hum > 0) {
    ...
    protected final void rotateAfterDropIfConfigured(double droppedY) {
        int yawDelta = TfmConfig.MACRO_DROP_ROTATION_DEGREES.get();
        if (droppedY <= 1.5 || !TfmConfig.MACRO_ROTATE_ON_DROP.get() || yaw.isEmpty() || yawDelta == 0) {
            return;
        }
