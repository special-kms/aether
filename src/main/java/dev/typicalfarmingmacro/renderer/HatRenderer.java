...
public final class HatRenderer {
    ...
    public static boolean hasVisibleEffect() {
        ...
        return TfmConfig.HAT_ENABLED.get() && shouldRender(Minecraft.getInstance());
    }

    public static void render(LevelRenderContext ctx) {
        if (StreamerModeManager.isEnabled()) return;
        if (!TfmConfig.HAT_ENABLED.get()) return;

        Minecraft mc = Minecraft.getInstance();
        if (mc.level == null || mc.player == null || !shouldRender(mc)) return;

        int vertices = Math.max(3, Math.min(20, TfmConfig.HAT_VERTICES.get()));
        double radius = TfmConfig.HAT_RADIUS.get();
        double height = TfmConfig.HAT_HEIGHT.get();
        double yOffset = TfmConfig.HAT_Y_OFFSET.get();
    ...
    private static void drawFace(LevelRenderContext ctx, Vec3 cameraPos, Vec3 apex, Vec3 left, Vec3 right, float time,
            float offset) {
        if (!TfmConfig.HAT_FILLED.get()) return;
