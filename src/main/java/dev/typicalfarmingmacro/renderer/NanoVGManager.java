package dev.typicalfarmingmacro.renderer;

import com.mojang.blaze3d.opengl.DirectStateAccess;
import com.mojang.blaze3d.opengl.GlStateManager;
import com.mojang.blaze3d.opengl.GlTexture;
import com.mojang.blaze3d.systems.RenderSystem;
import dev.typicalfarmingmacro.mixin.AccessorGlDevice;
import dev.typicalfarmingmacro.mixin.AccessorGpuDevice;
...
public final class NanoVGManager {
    ...
    private static final String UNICODE_FALLBACK_FONT = "Tfm-Unicode-Fallback";
    ...
    public static void init() {
        ...
        if (vg == -1L) {
            throw new RuntimeException("[Tfm] Failed to create NanoVG context");
        }

        renderer = new NVGRenderer(vg);

        // Load bundled fonts
        loadFont("Inter-Regular", "/assets/typicalfarmingmacro/fonts/Inter-Regular.otf");
        loadFont("Inter-Bold",    "/assets/typicalfarmingmacro/fonts/Inter-Bold.otf");
        loadFont("Inter-Mono",    "/assets/typicalfarmingmacro/fonts/Inter-Mono.otf");
    ...
    public static void beginFrame(float width, float height) {
        if (!initialized) throw new IllegalStateException("[Tfm] NanoVGManager.init() must be called first");
        if (drawing)      throw new IllegalStateException("[Tfm] endFrame() was not called before beginFrame()");
        ...
        try {
            ...
            System.err.println("[Tfm] Failed to bind main render target for NanoVG: " + e.getMessage());
            savedFbo = GL30.glGetInteger(GL30.GL_FRAMEBUFFER_BINDING);
            overrideTargetFbo = -1;
        }
    ...
    public static void endFrame() {
        if (!drawing) throw new IllegalStateException("[Tfm] beginFrame() was not called before endFrame()");
    ...
    private static DirectStateAccess resolveDirectStateAccess() {
        return ((AccessorGlDevice) ((AccessorGpuDevice) RenderSystem.getDevice()).typicalfarmingmacro$getBackend())
                .typicalfarmingmacro$directStateAccess();
    }
     ...
     * @param resourcePath absolute path within the jar, e.g. {@code "/assets/typicalfarmingmacro/fonts/Inter-Regular.otf"}
     */
    public static void loadFont(String name, String resourcePath) {
        if (fontIds.containsKey(name)) return;
        try (InputStream in = TfmResources.open(resourcePath)) {
            if (in == null) {
                System.err.println("[Tfm] Font resource not found: " + resourcePath);
                return;
            }
            ...
            if (id == -1) {
                System.err.println("[Tfm] NanoVG failed to load font: " + name);
                return;
            }
            fontIds.put(name, id);
            fontBuffers.put(name, buffer); // prevent GC
        } catch (IOException e) {
            System.err.println("[Tfm] IOException loading font " + name + ": " + e.getMessage());
        }
    }
    ...
    private static void loadUnicodeFallbackFont() {
        ...
        if (!loadedAny) {
            System.err.println("[Tfm] No Unicode fallback font found; some glyphs may render as missing.");
        }
    }
    ...
    private static int loadFontFromFile(String name, String filePath) {
        ...
        try {
            ...
            int id = NanoVG.nvgCreateFontMem(vg, name, buffer, false);
            if (id == -1) {
                System.err.println("[Tfm] NanoVG failed to load font: " + filePath);
                return -1;
            }
            fontIds.put(name, id);
            ...
            System.err.println("[Tfm] IOException loading font " + filePath + ": " + e.getMessage());
            return -1;
        }
    }
