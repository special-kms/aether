...
public final class RippleEffect {
    ...
    private void initShaderAndQuad() {
        int v = compile(GL20.GL_VERTEX_SHADER,   load("/assets/typicalfarmingmacro/shaders/ripple.vsh"));
        int f = compile(GL20.GL_FRAGMENT_SHADER, load("/assets/typicalfarmingmacro/shaders/ripple.fsh"));
        ...
        if (GL20.glGetProgrami(program, GL20.GL_LINK_STATUS) == GL11.GL_FALSE)
            throw new RuntimeException("[Tfm] RippleEffect link: " + GL20.glGetProgramInfoLog(program));
    ...
    private static int compile(int type, String src) {
        ...
        if (GL20.glGetShaderi(shader, GL20.GL_COMPILE_STATUS) == GL11.GL_FALSE)
            throw new RuntimeException("[Tfm] RippleEffect compile: " + GL20.glGetShaderInfoLog(shader));
        return shader;
    }

    private static String load(String path) {
        try (InputStream in = TfmResources.open(path)) {
            if (in == null) throw new RuntimeException("[Tfm] RippleEffect: missing " + path);
            return new String(in.readAllBytes(), StandardCharsets.UTF_8);
        } catch (IOException e) {
            throw new RuntimeException("[Tfm] RippleEffect: failed to read " + path, e);
        }
    }
}
