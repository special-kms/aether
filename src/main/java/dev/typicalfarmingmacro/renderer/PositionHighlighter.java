...
import dev.typicalfarmingmacro.config.RewarpPointPair;
import dev.typicalfarmingmacro.config.RewarpPointPairs;
...
public final class PositionHighlighter {
    ...
    public static boolean hasVisibleHighlights() {
        ...
        return dev.typicalfarmingmacro.modules.metaldetector.MetalDetectorSolver.hasVisibleHighlights();
    }
    ...
    private static boolean hasVisibleGardenHighlights() {
        ...
        if (TfmConfig.PEST_HIGHLIGHT_DESK.get()) {
            return true;
        }
        if (TfmConfig.AUTO_COMPOSTER_HIGHLIGHT.get()) {
            return true;
        }
        ...
        if (!dev.typicalfarmingmacro.modules.GreenhouseManager.highlightedSkulls.isEmpty()) {
            return true;
        }
        List<Vec3> path = dev.typicalfarmingmacro.modules.GreenhouseManager.currentPath;
        if (path != null && !path.isEmpty()) {
            return true;
        }
        if (dev.typicalfarmingmacro.modules.GreenhouseManager.getCurrentTarget() != null) {
            return true;
        }
        if (dev.typicalfarmingmacro.modules.GreenhouseManager.getCurrentAimTarget() != null) {
            return true;
        }
        if (dev.typicalfarmingmacro.modules.farming.FastLaneSwitchManager.hasVisibleHighlights()) {
            return true;
        }
        return false;
    }
    ...
    public static void renderWorld(LevelRenderContext ctx) {
        ...
        if (inGarden) {
            ...
            if (TfmConfig.PEST_HIGHLIGHT_DESK.get()) {
                int x = TfmConfig.PEST_EXCHANGE_DESK_X.get();
                int y = TfmConfig.PEST_EXCHANGE_DESK_Y.get();
                int z = TfmConfig.PEST_EXCHANGE_DESK_Z.get();
            ...
            if (TfmConfig.AUTO_COMPOSTER_HIGHLIGHT.get()) {
                int x = TfmConfig.AUTO_COMPOSTER_X.get();
                int y = TfmConfig.AUTO_COMPOSTER_Y.get();
                int z = TfmConfig.AUTO_COMPOSTER_Z.get();
            ...
            // Greenhouse Skulls
            for (net.minecraft.world.phys.AABB box : dev.typicalfarmingmacro.modules.GreenhouseManager.highlightedSkulls) {
                renderBlockHighlight(ctx, mc, textBuffer,
                        box,
                        "Greenhouse Skull",
            ...
            // Greenhouse Path
            var path = dev.typicalfarmingmacro.modules.GreenhouseManager.currentPath;
            if (path != null && !path.isEmpty()) {
            ...
            var target = dev.typicalfarmingmacro.modules.GreenhouseManager.getCurrentTarget();
            if (target != null) {
                renderBlockHighlight(ctx, mc, textBuffer,
            ...
            var aimTarget = dev.typicalfarmingmacro.modules.GreenhouseManager.getCurrentAimTarget();
            if (aimTarget != null) {
                renderBlockHighlight(ctx, mc, textBuffer,
            ...
            dev.typicalfarmingmacro.modules.farming.FastLaneSwitchManager.renderWorld();
        }

        dev.typicalfarmingmacro.modules.metaldetector.MetalDetectorSolver.renderWorld();
    }
