...
public class RewarpPointPair {
    ...
    public static RewarpPointPair fromLegacyConfig() {
        ...
        pair.startX = TfmConfig.REWARP_START_X.get();
        pair.startY = TfmConfig.REWARP_START_Y.get();
        pair.startZ = TfmConfig.REWARP_START_Z.get();
        pair.startSet = TfmConfig.REWARP_START_POS_SET.get();
        pair.highlightStart = TfmConfig.REWARP_HIGHLIGHT_START.get();
        pair.endX = TfmConfig.REWARP_END_X.get();
        pair.endY = TfmConfig.REWARP_END_Y.get();
        pair.endZ = TfmConfig.REWARP_END_Z.get();
        pair.endSet = TfmConfig.REWARP_END_POS_SET.get();
        pair.highlightEnd = TfmConfig.REWARP_HIGHLIGHT_END.get();
        pair.rewarpMode = TfmConfig.ENABLE_PLOT_TP_REWARP.get() ? RewarpMode.PLOT_TP : RewarpMode.FLY;
        pair.plotTpNumber = TfmConfig.PLOT_TP_NUMBER.get();
        pair.holdWUntilWall = TfmConfig.HOLD_W_UNTIL_WALL.get();
        pair.aotvAlign = TfmConfig.REWARP_AOTV_ALIGN.get();
        return pair;
    }
