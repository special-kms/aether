...
import dev.typicalfarmingmacro.ui.settings.SettingType;
...
final class MainGUITextLayout {
    ...
    int wrappedSettingLabelLineCount(Setting setting, float rowW) {
        return estimateWrappedLineCount(TfmLang.localize(setting.getName()), settingLabelMaxWidth(setting, rowW), settingLabelFontSize(setting));
    }
    ...
    List<String> wrapSettingLabel(NVGRenderer nvg, Setting setting, float rowW) {
        return wrapTextToWidth(nvg, TfmLang.localize(setting.getName()), Fonts.REGULAR, settingLabelFontSize(setting), settingLabelMaxWidth(setting, rowW));
    }
