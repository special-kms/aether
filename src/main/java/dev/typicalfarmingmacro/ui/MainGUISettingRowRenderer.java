...
import dev.typicalfarmingmacro.ui.settings.Setting;
...
final class MainGUISettingRowRenderer {
    ...
    private void renderText(NVGRenderer nvg, TextSetting setting, float x, float w, float y, float cardH,
        ...
        if (display.isEmpty() && !active) {
            nvg.text(Fonts.REGULAR, TfmLang.localize(setting.getPlaceholder()), fieldX + textPad, fieldY + (fieldH - fieldTextSize) / 2f, fieldTextSize, Theme.TEXT_DIM);
        } else {
    ...
    private void renderMultilineText(NVGRenderer nvg, TextSetting setting, float x, float w, float y, float cardH,
        ...
        if (display.isEmpty() && !active) {
            nvg.text(Fonts.REGULAR, TfmLang.localize(setting.getPlaceholder()), x + textPadX, fieldY + textPadY, fieldTextSize, Theme.TEXT_DIM);
        }
    ...
    private void renderList(NVGRenderer nvg, ListSetting setting, float x, float w, float y,
        ...
        for (int i = 0; i < rowCount; i++) {
            ...
            if (active) {
                ...
                nvg.text(Fonts.REGULAR, TfmLang.localize(setting.getPlaceholder()), x + textPad,
    ...
    private void renderAction(NVGRenderer nvg, ActionSetting setting, float x, float w, float y, float cardH, float mx, float my) {
        ...
        nvg.textCentered(Fonts.REGULAR, TfmLang.localize(setting.getName()), x, buttonY, w, buttonH, 12.5f, buttonText);
    }
    ...
    private void renderPosition(NVGRenderer nvg, PositionSetting setting, float x, float w, float y, float cardH, float midY,
        ...
        nvg.textRight(Fonts.REGULAR, TfmLang.localize("Highlight"), pillX - highlightW - 8f, midY - labelFontSize / 2f,
        ...
        nvg.textCentered(Fonts.REGULAR, TfmLang.localize("Capture"), captureX, boxY, captureW, boxH, 11.5f, buttonText);
    }
    ...
    private void renderMultiDropdown(NVGRenderer nvg, MultiDropdownSetting setting, float x, float w, float y, float cardH,
        ...
        for (int i = 0; i < options.size(); i++) {
            String optionText = TfmLang.localize(options.get(i));
        ...
        for (int i = 0; i < options.size(); i++) {
            ...
            nvg.textCentered(Fonts.REGULAR, TfmLang.localize(options.get(i)), cx, chipY, chipWidths[i], chipH, chipFontSz, textColor);
    ...
    private void renderKeybind(NVGRenderer nvg, KeybindSetting setting, float x, float w, float y, float cardH,
        ...
        String bindTextValue = listening ? TfmLang.localize("Press a key...") : setting.getBoundKeyName();
        float textPad = 8f;
        ...
        nvg.textCentered(Fonts.REGULAR, TfmLang.localize("Reset"), resetX, buttonY, resetW, buttonH, 12f, resetText);
    }
}
