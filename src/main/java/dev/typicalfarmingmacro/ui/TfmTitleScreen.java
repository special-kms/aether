 ...
 *
 * <p>Injected via {@link dev.typicalfarmingmacro.mixin.MixinTitleScreen}. All animation state
 * is driven by wall-clock time ({@link System#currentTimeMillis()}).</p>
 */
public class TfmTitleScreen extends NVGScreen {
    ...
    private static final String VERSION = FabricLoader.getInstance()
            .getModContainer("typicalfarmingmacro")
            .map(c -> c.getMetadata().getVersion().getFriendlyString())
            .orElse("dev");


    public TfmTitleScreen() { super("Tfm"); }
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mx, int my, float partialTick) {
        ...
        TfmRenderQueue.enqueue(() -> renderQueued(mx, my, elapsed));
    }
    ...
    private void renderQueued(int mx, int my, long elapsed) {
        ...
        TfmBackground.INSTANCE.render(width, height, mx, my);
    ...
    private void drawAll(NVGRenderer nvg, long elapsed) {
        ...
        if (aTitle > 0f) {
            ...
            float tSize = 36f;
            float tW    = nvg.textWidth(Fonts.BOLD, "Tfm", tSize);
            float tx    = cx - tW / 2f;
            nvg.text(Fonts.BOLD, "Tfm", tx, titleY, tSize, 0xFFFFFFFF);
        ...
        if (aBtm > 0f) {
            ...
            String user = TfmLang.localize("Logged in as ") + Minecraft.getInstance().getUser().getName();
    ...
    private void drawProxyButton(NVGRenderer nvg, int accent) {
        ...
        nvg.textCentered(Fonts.REGULAR, TfmProxyManager.selectedStatus(), bx, proxyY, PROXY_W, PROXY_H, 12f,
    ...
    public boolean mouseClicked(MouseButtonEvent click, boolean doubled) {
        ...
        if (mx >= proxyX && mx <= proxyX + PROXY_W && my >= proxyY && my <= proxyY + PROXY_H) {
            Minecraft.getInstance().setScreen(new TfmProxyScreen(this));
            return true;
        }
        return true;
    }
