...
final class MainGUISearchPanel {
    ...
    void render(NVGRenderer nvg, float mx, float my) {
        ...
        for (SearchResult result : searchResults) {
            if (result.group() != lastGroup) {
                ...
                nvg.text(Fonts.BOLD, TfmLang.localize(result.group().getName()), gx + 12f, y + 9f, 13f, Theme.TEXT_PRIMARY);
                String ctx = TfmLang.localize(result.tabLabel()) + " > " + TfmLang.localize(result.subtabLabel());
        ...
        if (searchResults.isEmpty()) {
            nvg.textCentered(
                    Fonts.REGULAR,
                    TfmLang.localize("No results for") + " \"" + owner.searchQuery + "\"",
                    owner.contX,
                    y + 16f,
                    owner.contW,
                    20f,
    ...
    private void buildResults(String query) {
        ...
        for (MainGUIRegistry.ModuleSection section : MainGUIRegistry.MODULE_SECTIONS) {
            addResults(searchResults, query, TfmLang.localize(section.displayName()), section.subtabs());
        }
        addResults(searchResults, query, TfmLang.localize("Colors"), MainGUIRegistry.COLORS_SUBTABS);
        addResults(searchResults, query, TfmLang.localize("Keybinds"), MainGUIRegistry.KEYBINDS_SUBTABS);
        addResults(searchResults, query, TfmLang.localize("Settings"), MainGUIRegistry.SETTINGS_SUBTABS);
    }
