...
import dev.typicalfarmingmacro.config.ConfigProfileManager;
import dev.typicalfarmingmacro.config.ThemeProfileManager;
...
final class MainGUIProfilesPanel {
    ...
    void commitRename() {
        ...
        if (renamed) {
            String title = isConfig ? TfmLang.localize("Config Renamed") : TfmLang.localize("Theme Renamed");
            NotificationManager.success(title, "\"" + oldName + "\" renamed to \"" + newName + "\"");
        } else {
            String title = isConfig ? TfmLang.localize("Config Rename Failed") : TfmLang.localize("Theme Rename Failed");
    ...
    void render(NVGRenderer nvg, float mx, float my) {
        ...
        if (owner.showConfigProfiles()) {
            y += 14f;
            tot += 14f;
            LayoutCursor cursor = renderSection(nvg, mx, my, gx, gw, y, tot, true, TfmLang.localize("Config Profiles"));
            y = cursor.y();
            tot = cursor.tot();
        }
        if (owner.showThemeProfiles()) {
            y += 14f;
            tot += 14f;
            LayoutCursor cursor = renderSection(nvg, mx, my, gx, gw, y, tot, false, TfmLang.localize("Theme Profiles"));
            y = cursor.y();
            tot = cursor.tot();
        }
    ...
    private LayoutCursor renderSection(
        ...
        nvg.text(Fonts.BOLD, TfmLang.localize("Save Profile"), gx, y, 10f, Theme.TEXT_MUTED);
        y += 16f;
        tot += 16f;
        ...
        String nameDisp = owner.profileNameInput.isEmpty() && !nameFocused ? TfmLang.localize("Profile name...") : owner.profileNameInput;
        ...
        nvg.textCentered(Fonts.REGULAR, TfmLang.localize("Save"), saveBtnX, y, saveBtnW, fieldH, 12.5f, saveTxt);
        ...
        owner.addClickArea(saveBtnX, y, saveBtnW, fieldH, () -> {
            if (!capturedName.isBlank()) {
                if (isConfig) {
                    ...
                    NotificationManager.success(TfmLang.localize("Config Saved"), "\"" + capturedName + "\" saved successfully");
                } else {
                    ThemeProfileManager.save(capturedName);
                    NotificationManager.success(TfmLang.localize("Theme Saved"), "\"" + capturedName + "\" saved successfully");
                }
        ...
        nvg.text(Fonts.BOLD, TfmLang.localize("Saved Profiles"), gx, y, 10f, Theme.TEXT_MUTED);
        y += 14f;
        tot += 14f;
        ...
        if (profiles.isEmpty()) {
            ...
            nvg.textCentered(Fonts.REGULAR, TfmLang.localize("No saved profiles yet."), gx, y, gw, emptyH, 11f, Theme.TEXT_MUTED);
            y += emptyH + 4f;
            tot += emptyH + 4f;
        } else {
            float rowH = 44f;
            ...
            for (String name : profiles) {
                ...
                for (int index = 0; index < 3; index++) {
                    ...
                    String label = switch (index) {
                        case 0 -> TfmLang.localize("Load");
                        case 1 -> TfmLang.localize("Export");
                        default -> TfmLang.localize("Delete");
                    };
                            ...
                            case 0 -> {
                                if (isConfig) {
                                    if (ConfigProfileManager.load(profileName)) {
                                        ...
                                        NotificationManager.success(TfmLang.localize("Config Loaded"), "\"" + profileName + "\" loaded successfully");
                                    } else {
                                        NotificationManager.error(TfmLang.localize("Config Load Failed"), "\"" + profileName + "\" could not be loaded");
                                    }
                                } else {
                                    ThemeProfileManager.load(profileName);
                                    NotificationManager.success(TfmLang.localize("Theme Loaded"), "\"" + profileName + "\" loaded successfully");
                                }
                            }
                            case 1 -> {
                                ...
                                NotificationManager.info(TfmLang.localize("Copied to Clipboard"), "\"" + profileName + "\" exported");
                            }
                            case 2 -> {
                                if (isConfig) {
                                    ConfigProfileManager.delete(profileName);
                                    NotificationManager.warning(TfmLang.localize("Config Deleted"), "\"" + profileName + "\" was deleted");
                                } else {
                                    ThemeProfileManager.delete(profileName);
                                    NotificationManager.warning(TfmLang.localize("Theme Deleted"), "\"" + profileName + "\" was deleted");
                                }
                            }
                        }
                    });
        ...
        nvg.textCentered(Fonts.REGULAR, TfmLang.localize("Import from Clipboard"), gx, y, importW, importH, 12.5f, importTxt);
