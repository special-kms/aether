...
import dev.typicalfarmingmacro.notification.NotificationRenderer;
import dev.typicalfarmingmacro.ui.settings.*;
...
import dev.typicalfarmingmacro.renderer.NVGScreen;
...
public class MainGUI extends NVGScreen {
    ...
    private static final String[] TAB_DESCS = {
            ...
            "Edit Tfm keybinds and keep them synced with Minecraft controls",
    ...
    private float renderPetTrackerHeaderButtons(NVGRenderer nvg, SettingGroup group, float right, float btnY,
        ...
        if (canRemove) {
            ...
            nvg.textCentered(Fonts.REGULAR, TfmLang.localize("Remove"), bx, btnY, btnW, btnH, 11f, Theme.TEXT_PRIMARY);
        ...
        if (canAdd) {
            ...
            nvg.roundedRect(bx, btnY, btnW, btnH, 4f, hovered ? Theme.BG_HOVER : Theme.BG_FIELD);
            nvg.rectOutline(bx, btnY, btnW, btnH, 4f, 1f, Theme.BORDER_DEFAULT);
            nvg.textCentered(Fonts.REGULAR, TfmLang.localize("Add"), bx, btnY, btnW, btnH, 11f, Theme.TEXT_PRIMARY);
            clickAreas.add(new ClickArea(bx, btnY, btnW, btnH, () -> {
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mx, int my, float dt) {
        ...
        TfmRenderQueue.enqueue(this::renderQueuedFrame);
    }
    ...
    private static String moduleCardName(ModulesTab.SubTab sub) {
        return TfmLang.localize(sub.name());
    }
    ...
    private void renderGroupHeader(NVGRenderer nvg, SettingGroup group,
        ...
        nvg.text(Fonts.BOLD,    TfmLang.localize(group.getName()),        x + 12f, y +  9f, 13f, titleColor);
        nvg.text(Fonts.REGULAR, TfmLang.localize(group.getDescription()), x + 12f, y + 26f, 10f, descColor);
    ...
    private String[] localizedStrings(String[] values) {
        ...
        for (int i = 0; i < values.length; i++) {
            localized[i] = TfmLang.localize(values[i]);
        }
        return localized;
    }
    ...
    private String[] moduleFilterOptions() {
        ...
        if (sections.size() == 1) {
            return new String[] { TfmLang.localize(sections.getFirst().name()) };
        }

        String[] options = new String[sections.size() + 1];
        options[0] = TfmLang.localize("All");
        for (int i = 0; i < sections.size(); i++) {
            options[i + 1] = TfmLang.localize(sections.get(i).name());
        }
        return options;
    }
    ...
    private ModulesTab.SubTab findModuleSubTabByName(String name) {
        ...
        for (ModulesTab.SubTab subTab : MainGUIRegistry.MODULE_SUBTABS) {
            if (subTab.name().equalsIgnoreCase(name) || TfmLang.localize(subTab.name()).equalsIgnoreCase(name)) {
                return subTab;
            }
        }
        return null;
    ...
    List<MainGUIModuleSection> moduleSectionsForFilter() {
        ...
        for (Section section : sectionsForFilter()) {
            result.add(new MainGUIModuleSection(TfmLang.localize(section.name()), section.subtabs()));
        }
        return result;
    }
    ...
    private void renderOptionalFeatureHud(NVGRenderer nvg, float dt) {
        TfmBootstrapHooks.renderConfigScreenOverlay(nvg, (float) width, (float) height, dt);
        NotificationRenderer.render(nvg, (float) width, (float) height, dt);
    }

    private void openOptionalHudEditor(Minecraft minecraft) {
        net.minecraft.client.gui.screens.Screen screen = TfmBootstrapHooks.maybeCreateHudEditScreen();
        if (screen != null) {
