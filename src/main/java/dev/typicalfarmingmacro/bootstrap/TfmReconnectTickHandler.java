...
public final class TfmReconnectTickHandler {
    ...
    private TfmReconnectTickHandler() {
    }
            ...
            if (reconnectAt != 0) {
                if (ReconnectScheduler.shouldResume()) {
                    if (ReconnectScheduler.getReconnectMode() == ReconnectScheduler.ReconnectMode.PROXY_RESTART) {
                        dev.typicalfarmingmacro.modules.session.RecoveryManager.beginRecovery(
                                dev.typicalfarmingmacro.modules.session.RecoveryManager.RecoveryMode.PROXY_RESTART);
                    } else {
                        dev.typicalfarmingmacro.modules.session.RecoveryManager.beginRecovery(
                                dev.typicalfarmingmacro.modules.session.RecoveryManager.RecoveryMode.STANDARD);
                    }
