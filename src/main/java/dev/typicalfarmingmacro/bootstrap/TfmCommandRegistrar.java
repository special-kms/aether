...
import dev.typicalfarmingmacro.modules.forge.ForgeManager;
import dev.typicalfarmingmacro.modules.failsafe.FailsafeTestManager;
import dev.typicalfarmingmacro.modules.interaction.EntityInteractManager;
...
import dev.typicalfarmingmacro.modules.pest.helpers.PestTrapManager;
import dev.typicalfarmingmacro.modules.rotation.RotationManager;
import dev.typicalfarmingmacro.modules.visitor.VisitorsMacro;
import dev.typicalfarmingmacro.util.TfmLang;
import dev.typicalfarmingmacro.util.BazaarUtils;
...
public final class TfmCommandRegistrar {
    private TfmCommandRegistrar() {
    }
    ...
    private static void registerLegacyCommandIntercepts() {
        ...
        ClientSendMessageEvents.COMMAND.register(command -> {
            ...
            if (command.equalsIgnoreCase("call george")) {
                dev.typicalfarmingmacro.modules.inventorymanager.GeorgeManager.onCallGeorgeSent();
            }

            String normalized = command.toLowerCase().trim();
            if (normalized.startsWith("typicalfarmingmacro goto ")) {
                startCoordinatePathfind(command, false, "\u00A7eUsage: /typicalfarmingmacro goto <x> <y> <z>");
            }
            if (normalized.startsWith("typicalfarmingmacro flyto ")) {
                startCoordinatePathfind(command, true, "\u00A7eUsage: /typicalfarmingmacro flyto <x> <y> <z>");
            }
            if (normalized.equals("typicalfarmingmacro debug path")) {
                PathVisualizer.toggle();
                ClientUtils.sendMessage(Minecraft.getInstance(),
                        "\u00A7aPath visualizer: " + (PathVisualizer.isEnabled() ? "ON" : "OFF"), false);
            }
            if (normalized.startsWith("typicalfarmingmacro pathtest ")) {
    ...
    private static void registerClientCommands() {
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            dispatcher.register(
                    ClientCommands.literal("typicalfarmingmacro")
                            .executes(ctx -> {
                                TfmUiActions.toggleMainGui(Minecraft.getInstance());
                                return 1;
                            })
                            .then(ClientCommands.literal("farming")
                                    .executes(ctx -> {
                                        ...
                                        TfmKeybindHandler.startFarmingMacro(client);
                                        return 1;
                                    }))
                            ...
                            .then(ClientCommands.literal("bazaar")
                                    .executes(ctx -> {
                                        ClientUtils.sendMessage(Minecraft.getInstance(),
                                                "\u00A7eUsage: /typicalfarmingmacro bazaar <item> <count>", false);
                                        return 1;
                                    })
    ...
    private static void startPathTest(String command) {
        ...
        try {
            ...
            ClientUtils.sendMessage(Minecraft.getInstance(), "\u00A7eUsage: /typicalfarmingmacro pathtest <x> <y> <z>", false);
        }
    }

    private static void sendPathfindHelp(Minecraft client) {
        ClientUtils.sendMessage(client, "\u00A7ePathfind commands:", false);
        ClientUtils.sendMessage(client, "\u00A77  /typicalfarmingmacro pathfind \u00A7fx y z \u00A78- walk to coords", false);
        ClientUtils.sendMessage(client, "\u00A77  /typicalfarmingmacro pathfind \u00A7fwalk x y z \u00A78- walk to coords", false);
        ClientUtils.sendMessage(client, "\u00A77  /typicalfarmingmacro pathfind \u00A7ffly x y z \u00A78- fly to coords", false);
        ClientUtils.sendMessage(client, "\u00A77  /typicalfarmingmacro pathfind \u00A7fetherwarp x y z \u00A78- etherwarp to coords", false);
        ClientUtils.sendMessage(client, "\u00A77  /typicalfarmingmacro pathfind \u00A7fstop \u00A78- stop pathfinding", false);
    }

    private static void sendFailsafeTestHelp(Minecraft client) {
        ClientUtils.sendMessage(client, "\u00A7eFailsafe test commands:", false);
        ClientUtils.sendMessage(client, "\u00A77  /typicalfarmingmacro testfailsafe \u00A7finventoryslot <slot> \u00A78- switch hotbar slot", false);
        ClientUtils.sendMessage(client, "\u00A77  /typicalfarmingmacro testfailsafe \u00A7frotation <pitch> <yaw>", false);
        ClientUtils.sendMessage(client, "\u00A77  /typicalfarmingmacro testfailsafe \u00A7fguiflash <durationTicks>", false);
    }

    private static void sendConfigHelp(Minecraft client) {
        ClientUtils.sendMessage(client, "\u00A7eConfig commands:", false);
        ClientUtils.sendMessage(client,
                "\u00A77  /typicalfarmingmacro config \u00A7fexport \u00A78- copy your config to the clipboard", false);
        ClientUtils.sendMessage(client,
                "\u00A77  /typicalfarmingmacro config \u00A7fimport <string> \u00A78- apply a pasted config string", false);
    }

    private static int exportConfig(Minecraft client) {
        String json = TfmConfig.exportSanitizedJson();
    ...
    private static int importConfig(Minecraft client, String json) {
        if (TfmConfig.importFromJson(json)) {
            TfmBootstrapHooks.onConfigProfileLoaded(TfmConfig.getConfigFile());
    ...
    private static void sendMovementHelp(Minecraft client) {
        ...
        ClientUtils.sendMessage(client, "\u00A77  /typicalfarmingmacro movement \u00A7frecord \u00A78- record movement events", false);
        ClientUtils.sendMessage(client, "\u00A77  /typicalfarmingmacro movement \u00A7fstop \u00A78- stop recording or playback", false);
        ClientUtils.sendMessage(client, "\u00A77  /typicalfarmingmacro movement \u00A7ffolder \u00A78- open the movement folder", false);
        ClientUtils.sendMessage(client, "\u00A77  /typicalfarmingmacro movement \u00A7fplay <replay_file> \u00A78- play a recording", false);
    }
    ...
    private static int printScoreboard(Minecraft client) {
        ...
        if (lines.isEmpty()) {
            ClientUtils.sendMessage(client, "\u00A7c" + TfmLang.localize("No scoreboard lines found."), false);
            return 0;
        }

        ClientUtils.sendMessage(client,
                "\u00A7e" + TfmLang.localize("Scoreboard lines (%d):").formatted(lines.size()), false);
        for (int i = 0; i < lines.size(); i++) {
            String line = lines.get(i).isBlank() ? TfmLang.localize("[blank]") : lines.get(i);
    ...
    private static int triggerDynamicPest(Minecraft client, String crop) {
        ...
        if (DynamicPestsManager.triggerTestApply(client, crop)) {
            ClientUtils.sendMessage(client,
                    TfmLang.localize("Dynamic Pests test triggered for %s.").formatted(crop), false);
            return 1;
        }

        ClientUtils.sendMessage(client, TfmLang.localize("Unable to trigger Dynamic Pests test."), false);
        return 0;
    }
}
