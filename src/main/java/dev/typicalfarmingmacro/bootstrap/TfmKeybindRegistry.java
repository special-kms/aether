...
public final class TfmKeybindRegistry {
    ...
    public static void register() {
        ...
        Identifier categoryId = Identifier.fromNamespaceAndPath("typicalfarmingmacro", "main");
        KeyMapping.Category category = new KeyMapping.Category(categoryId);
        try {
            macroToggleKey = KeyMappingHelper
                    .registerKeyMapping(new KeyMapping("key.typicalfarmingmacro.start_script", GLFW.GLFW_KEY_K, category));
            clickGuiKey = KeyMappingHelper
                    .registerKeyMapping(new KeyMapping("key.typicalfarmingmacro.clickgui", GLFW.GLFW_KEY_INSERT, category));
            ...
            freecamKey = KeyMappingHelper
                    .registerKeyMapping(new KeyMapping("key.typicalfarmingmacro.freecam", GLFW.GLFW_KEY_F6, category));
            freecamTeleportToPlayerKey = KeyMappingHelper
                    .registerKeyMapping(new KeyMapping("key.typicalfarmingmacro.freecam_teleport_to_player", GLFW.GLFW_KEY_F7, category));
            freelookKey = KeyMappingHelper
                    .registerKeyMapping(new KeyMapping("key.typicalfarmingmacro.freelook", GLFW.GLFW_KEY_LEFT_ALT, category));
            pipKey = KeyMappingHelper
                    .registerKeyMapping(new KeyMapping("key.typicalfarmingmacro.pip", GLFW.GLFW_KEY_P, category));
            ungrabMouseKey = KeyMappingHelper
                    .registerKeyMapping(new KeyMapping("key.typicalfarmingmacro.ungrab_mouse", GLFW.GLFW_KEY_U, category));
        } catch (IllegalStateException ex) {
            // External feature jars can initialize after options are already built; reuse existing mappings if present.
            macroToggleKey = resolveExistingOrDetached("key.typicalfarmingmacro.start_script", GLFW.GLFW_KEY_K, category);
            clickGuiKey = resolveExistingOrDetached("key.typicalfarmingmacro.clickgui", GLFW.GLFW_KEY_INSERT, category);
            nvgDemoKey = resolveExistingOrDetached("Open NVG Demo", GLFW.GLFW_KEY_HOME, category);
            freecamKey = resolveExistingOrDetached("key.typicalfarmingmacro.freecam", GLFW.GLFW_KEY_F6, category);
            freecamTeleportToPlayerKey = resolveExistingOrDetached("key.typicalfarmingmacro.freecam_teleport_to_player", GLFW.GLFW_KEY_F7, category);
            freelookKey = resolveExistingOrDetached("key.typicalfarmingmacro.freelook", GLFW.GLFW_KEY_LEFT_ALT, category);
            pipKey = resolveExistingOrDetached("key.typicalfarmingmacro.pip", GLFW.GLFW_KEY_P, category);
            ungrabMouseKey = resolveExistingOrDetached("key.typicalfarmingmacro.ungrab_mouse", GLFW.GLFW_KEY_U, category);
        }
        registered = true;
    }
