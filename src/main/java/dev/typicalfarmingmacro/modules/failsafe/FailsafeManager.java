...
public final class FailsafeManager {
    ...
    public static double getExpectedBps() {
        return TfmConfig.FAILSAFE_BPS_THRESHOLD.get();
    }
    ...
    public static double getExpectedGhostBlockWindowSeconds() {
        return TfmConfig.FAILSAFE_GHOST_BLOCK_WINDOW_SECONDS.get();
    }
    ...
    public static long sampleAdditionalTriggerDelayMs() {
        float maxAdditionalDelaySeconds = TfmConfig.FAILSAFE_ADDITIONAL_RANDOM_DELAY_SECONDS.get();
    ...
    public static FailsafeAction getInventorySlotChangedAction() {
        return FailsafeAction.fromConfig(TfmConfig.FAILSAFE_INVENTORY_SLOT_CHANGED_ACTION.get());
    }

    public static FailsafeAction getUnexpectedInventoryGuiAction() {
        return FailsafeAction.fromConfig(TfmConfig.FAILSAFE_UNEXPECTED_INVENTORY_GUI_ACTION.get());
    }

    public static FailsafeAction getBpsAction() {
        return FailsafeAction.fromConfig(TfmConfig.FAILSAFE_BPS_ACTION.get());
    }

    public static FailsafeAction getGhostBlockAction() {
        return FailsafeAction.fromConfig(TfmConfig.FAILSAFE_GHOST_BLOCK_ACTION.get());
    }

    public static FailsafeAction getDirtCheckAction() {
        return FailsafeAction.fromConfig(TfmConfig.FAILSAFE_DIRT_CHECK_ACTION.get());
    }
    ...
    public static FailsafeAction getWorldChangeAction() {
        return FailsafeAction.fromConfig(TfmConfig.FAILSAFE_WORLD_CHANGE_ACTION.get());
    }
    ...
    public static void handleConfiguredAction(
        ...
        if (TfmConfig.FAILSAFE_DESKTOP_NOTIFICATION_ENABLED.get()) {
        ...
        if (action == FailsafeAction.CUSTOM) {
            ClientUtils.sendMessage(client,
                    "\u00A7eFailsafe triggered: " + details + " " + TfmLang.localize("Running custom replay."),
                    false);
    ...
    public static FailsafeAction getConfiguredAction() {
        return FailsafeAction.fromConfig(TfmConfig.FAILSAFE_ACTION.get());
    }
