...
final class BpsFailsafe {
    ...
    static void tick(Minecraft client) {
        ...
        double expectedBps = TfmConfig.FAILSAFE_BPS_THRESHOLD.get();
        ...
        long triggerDelayMs = Math.round(TfmConfig.FAILSAFE_BPS_TRIGGER_DELAY_SECONDS.get() * 1000.0f)
                + lowBpsRandomDelayMs;
    ...
    static State getState(Minecraft client) {
        ...
        if (client == null || client.player == null || !TfmConfig.FAILSAFE_BPS.get() || !isTrackingActive()) {
            return State.IDLE;
        }
        if (getWindowSeconds() < getConfiguredWindowSeconds()) {
            return State.WAIT;
        }
        if (getCurrentBps() < TfmConfig.FAILSAFE_BPS_THRESHOLD.get()) {
            return State.WAIT;
        }
        return State.IDLE;
    }
    ...
    private static long getConfiguredWindowMs() {
        return TfmConfig.FAILSAFE_BPS_WINDOW_SECONDS.get() * 1000L;
    }

    private static double getConfiguredWindowSeconds() {
        return TfmConfig.FAILSAFE_BPS_WINDOW_SECONDS.get();
    }
