...
final class GhostBlockFailsafe {
    ...
    static void tick(Minecraft client) {
        ...
        long totalDelayMs = getConfiguredWindowMs()
                + Math.round(TfmConfig.FAILSAFE_GHOST_BLOCK_TRIGGER_DELAY_SECONDS.get() * 1000.0f)
    ...
    static State getState(Minecraft client) {
        ...
        if (client == null || client.player == null || !TfmConfig.FAILSAFE_GHOST_BLOCK.get() || !isTrackingActive()) {
            return State.IDLE;
        }
    ...
    private static long getConfiguredWindowMs() {
        return TfmConfig.FAILSAFE_GHOST_BLOCK_WINDOW_SECONDS.get() * 1000L;
    }
