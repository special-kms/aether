...
public class GeorgeManager {
    ...
    private static dev.typicalfarmingmacro.macro.MacroState.State getPrepRequiredState() {
        return TfmConfig.FARM_WHILE_CALLING_GEORGE.get()
                ? dev.typicalfarmingmacro.macro.MacroState.State.FARMING
                : dev.typicalfarmingmacro.macro.MacroState.State.GEORGE;
    }

    private static void pauseFarmingForGeorgeIfNeeded() {
        if (!TfmConfig.FARM_WHILE_CALLING_GEORGE.get()
                && dev.typicalfarmingmacro.macro.MacroStateManager.getCurrentState() == dev.typicalfarmingmacro.macro.MacroState.State.FARMING) {
            dev.typicalfarmingmacro.macro.MacroStateManager.setCurrentState(dev.typicalfarmingmacro.macro.MacroState.State.GEORGE);
        }
    }

    private static void resumeFarmingAfterGeorgeWait(Minecraft client, String debugMessage) {
        if (TfmConfig.FARM_WHILE_CALLING_GEORGE.get()
                && dev.typicalfarmingmacro.macro.MacroStateManager.getCurrentState() == dev.typicalfarmingmacro.macro.MacroState.State.GEORGE) {
    ...
    public static void handleGeorgeMenu(Minecraft client, AbstractContainerScreen<?> screen) {
        ...
        if (!isSelling && !isPreparingToSell) {
            if (!dev.typicalfarmingmacro.macro.MacroStateManager.isMacroRunning()) {
                return;
            }
    ...
    private static boolean isPriorityEventActive(Minecraft client) {
        dev.typicalfarmingmacro.macro.MacroState.State state = dev.typicalfarmingmacro.macro.MacroStateManager.getCurrentState();
        if (state != dev.typicalfarmingmacro.macro.MacroState.State.FARMING && state != dev.typicalfarmingmacro.macro.MacroState.State.GEORGE) {
            return true;
        }
    ...
    public static void update(Minecraft client) {
        ...
        if (isPreparingToSell) {
            if (isPriorityEventActive(client)) {
                isPreparingToSell = false;
                if (dev.typicalfarmingmacro.macro.MacroStateManager.getCurrentState() == dev.typicalfarmingmacro.macro.MacroState.State.GEORGE) {
                    dev.typicalfarmingmacro.macro.MacroStateManager.setCurrentState(dev.typicalfarmingmacro.macro.MacroState.State.FARMING);
                }
            ...
            if (client.screen == null) {
                ...
                if (now - interactionTime > 1500) {
                    ...
                    if (remaining > 0) {
                        ...
                        confirmationCount = 0;
                        lastGeorgeSequenceActionTime = 0;
                        dev.typicalfarmingmacro.util.ClientUtils.sendCommand(client, "/call george");
                    } else {
                        finishSelling(client);
                    }
                }
        ...
        if (petCount >= TfmConfig.GEORGE_SELL_THRESHOLD.get()) {
    ...
    private static boolean shouldUseGeorgeAutomation() {
        dev.typicalfarmingmacro.macro.MacroState.State state = dev.typicalfarmingmacro.macro.MacroStateManager.getCurrentState();
        return TfmConfig.AUTO_GEORGE_SELL.get()
                && (state == dev.typicalfarmingmacro.macro.MacroState.State.FARMING
                        || state == dev.typicalfarmingmacro.macro.MacroState.State.GEORGE);
    }
