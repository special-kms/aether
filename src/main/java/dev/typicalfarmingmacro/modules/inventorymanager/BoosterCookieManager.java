...
public class BoosterCookieManager {
    ...
    public static void handleBoosterCookieMenu(Minecraft client, AbstractContainerScreen<?> screen) {
        if (!TfmConfig.AUTOSELL_PASSIVE.get() || screen == null || client.player == null)
            return;
        ...
        if (now - interactionTime < dev.typicalfarmingmacro.util.ClientUtils.getGuiClickDelayMs(interactionStage == 0))
            return;
        ...
        if (interactionStage == 0) {
            ...
            if (foundSlotIdx != -1) {
                dev.typicalfarmingmacro.util.ClientUtils.performSlotClick(client, screen, foundSlotIdx, 0, ContainerInput.QUICK_MOVE);
                interactionTime = now;
            }
        }
    }
