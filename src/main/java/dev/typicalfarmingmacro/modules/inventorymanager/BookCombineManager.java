...
public class BookCombineManager {
    ...
    public static void handleAnvilMenu(Minecraft client, AbstractContainerScreen<?> screen) {
        if (!TfmConfig.AUTO_BOOK_COMBINE.get() || screen == null || client.player == null)
            return;

        if (!TfmConfig.ALWAYS_ACTIVE_COMBINE.get() && !isCombining)
            return;

        long now = System.currentTimeMillis();
        long requiredDelay = interactionStage == 0
                ? ClientUtils.getGuiClickDelayMs(true)
                : ConfigHelpers.getRandomizedDelay(TfmConfig.BOOK_COMBINE_DELAY.get());
        ...
        if (interactionStage == 0) {
            ...
            for (Map.Entry<String, List<Integer>> entry : bookPairs.entrySet()) {
                ...
                dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "\u00A77BookCombine: combining " + key
                                + "' (slots " + pendingSlot0 + " + " + pendingSlot1 + ")", true);

                dev.typicalfarmingmacro.util.ClientUtils.performSlotClick(client, screen, pendingSlot0, 0, ContainerInput.QUICK_MOVE);
                interactionTime = now;
                interactionStage = 1;
                return;
            }
        ...
        // slots were frozen in stage 0 before any clicking)
            ...
            dev.typicalfarmingmacro.util.ClientUtils.performSlotClick(client, screen, pendingSlot1, 0, ContainerInput.QUICK_MOVE);
            interactionTime = now;
            interactionStage = 2;
        }
        // Stage 2: Pick up the combined result from the output slot (slot 22)
        else if (interactionStage == 2) {
            dev.typicalfarmingmacro.util.ClientUtils.performSlotClick(client, screen, 22, 0, ContainerInput.PICKUP);
            interactionTime = now;
            interactionStage = 3;
        }
    ...
    private static boolean isPriorityEventActive(Minecraft client) {
        return dev.typicalfarmingmacro.macro.MacroStateManager.getCurrentState() != dev.typicalfarmingmacro.macro.MacroState.State.FARMING
                ...
                || (TfmConfig.AUTO_DROP_JUNK.get() && JunkManager.countJunkItems(client) >= TfmConfig.JUNK_THRESHOLD.get());
    }

    public static void update(Minecraft client) {
        if (!TfmConfig.AUTO_BOOK_COMBINE.get() || client.player == null)
            return;

        if (isPreparingToCombine) {
            if (isPriorityEventActive(client)) {
                isPreparingToCombine = false;
                dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "\u00A7cAborting Book Combine prep due to priority event.", false);
            }
            return;
        }
        ...
        if (isCombining) {
            if (isPriorityEventActive(client)) {
                isCombining = false;
                dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "\u00A7cAborting Book Combine due to priority event.", false);
                return;
            }
        ...
        if (bookCount >= TfmConfig.BOOK_THRESHOLD.get()) {
            triggerAutomaticCombine(client, bookCount);
        }
    }

    private static void triggerAutomaticCombine(Minecraft client, int count) {
        dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "\u00A7eAuto combining books (" + count + " books in inventory)...", false);

        dev.typicalfarmingmacro.util.ClientUtils.forceReleaseKeys(client);
        ...
        MacroWorkerThread.getInstance().submit("BookCombine-Trigger", () -> {
            try {
                ...
                if (isPreparingToCombine) {
                    ...
                    dev.typicalfarmingmacro.util.ClientUtils.sendCommand(client, "/av");
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private static void finishCombining(Minecraft client) {
        ...
        if (client.player != null) {
            dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "\u00A7aBook Combine finished. Resuming script...", true);
        }
        ...
        if (dev.typicalfarmingmacro.macro.MacroStateManager.getCurrentState() == dev.typicalfarmingmacro.macro.MacroState.State.FARMING) {
            MacroWorkerThread.getInstance().submit("BookCombine-Finish", () -> {
                ...
                client.execute(() -> dev.typicalfarmingmacro.macro.FarmingMacroManager.enable(client, dev.typicalfarmingmacro.macro.FarmingMacroManager.createMacroFromConfig()));
            });
        }
    }
