package dev.typicalfarmingmacro.modules.discord;
...
public class DiscordStatusManager {
    private static final String SCREENSHOT_ATTACHMENT_PLACEHOLDER = "__TFM_SCREENSHOT__";
    ...
    public static void update(Minecraft client) {
        ...
        if (!isTakingScreenshot && now - lastUpdateTime >= TfmConfig.DISCORD_STATUS_UPDATE_TIME.get() * 60 * 1000L) {
            lastUpdateTime = now;
    ...
    public static boolean requestManualStatusUpdate(Minecraft client) {
        String webhookUrl = TfmConfig.DISCORD_WEBHOOK_URL.get();
    ...
    private static void sendScreenshotAsync(Minecraft client) {
        new Thread(() -> {
            try {
                ...
                sendWebhook(TfmConfig.DISCORD_WEBHOOK_URL.get(), latestScreenshot, payloadOverride);
                try {
    ...
    private static boolean isWebhookConfigured() {
        String webhookUrl = TfmConfig.DISCORD_WEBHOOK_URL.get();
        if (!TfmConfig.SEND_DISCORD_STATUS.get() || webhookUrl == null || webhookUrl.isEmpty()) {
            return false;
        }
        return true;
    }
    ...
    private static String buildStatusPayload(String imageFileName) {
        ...
        return "{\n" +
                "  \"embeds\": [{\n" +
                "    \"title\": \"Status Update\",\n" +
                "    \"description\": \"Here is your latest Tfm status update! :rocket:\",\n" +
                "    \"color\": 5814783,\n" +
                "    \"fields\": [\n" +
                "      {\n" +
                "        \"name\": \"Current State\",\n" +
    ...
    private static String buildFailsafePayload(String reason, String actionDone) {
        ...
        return "{\n" +
                ...
                "    \"description\": \"Tfm detected a failsafe and captured the current screen.\",\n" +
                "    \"color\": 15158332,\n" +
                "    \"fields\": [\n" +
                "      {\n" +
                "        \"name\": \"Current State\",\n" +
