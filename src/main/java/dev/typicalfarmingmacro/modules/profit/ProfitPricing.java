package dev.typicalfarmingmacro.modules.profit;
...
import dev.typicalfarmingmacro.modules.profit.helpers.PetXpTracker;
...
public final class ProfitPricing {
    ...
    public void printPetXpPriceDebug(Minecraft client) {
        ...
        for (String petConfig : TfmConfig.PET_TRACKER_LIST.get()) {
    ...
    public void handlePriceSourceChanged() {
        onPricesChanged.run();
        if (ProfitPriceSource.fromConfig(TfmConfig.PROFIT_PRICE_SOURCE.get()) == ProfitPriceSource.BAZAAR) {
            fetchBazaarPrices();
        }
    }
    ...
    public String fetchIdByName(String name) {
        ...
        try {
            ...
            System.err.println("[Tfm] Cofl item ID lookup failed for '" + name + "': " + e.getMessage());
        }
    ...
    private double getPreferredPrice(String lookupKey) {
        ...
        ProfitPriceSource source = ProfitPriceSource.fromConfig(TfmConfig.PROFIT_PRICE_SOURCE.get());
    ...
    private synchronized void fetchBazaarPrices() {
        ...
        new Thread(() -> performFetchInternal(HttpClient.newHttpClient()), "Tfm-Profit-Pricing").start();
    }
