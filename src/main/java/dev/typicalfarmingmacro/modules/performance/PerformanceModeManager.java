package dev.typicalfarmingmacro.modules.performance;
...
public class PerformanceModeManager {
    ...
    public static void start(Minecraft mc) {
        if (!TfmConfig.PERFORMANCE_MODE.get()) return;
        if (enabled) return;
        ...
        if (mc.options != null) {
            ...
            if (TfmConfig.PERFORMANCE_LIMIT_CHUNK_DISTANCE.get()) {
                mc.options.renderDistance().set(TfmConfig.PERFORMANCE_CHUNK_DISTANCE.get());
                changedRenderDistance = true;
            }
            if (TfmConfig.PERFORMANCE_LIMIT_FPS.get()) {
                mc.options.framerateLimit().set(TfmConfig.PERFORMANCE_MODE_MAX_FPS.get());
                changedMaxFps = true;
            }
            if (TfmConfig.PERFORMANCE_DISABLE_PARTICLES.get()) {
    ...
    public static boolean isParticlesDisabled() {
        return enabled && TfmConfig.PERFORMANCE_DISABLE_PARTICLES.get();
    }
}
