package dev.typicalfarmingmacro.modules.rotation;
...
public class RotationManager {
    ...
    public static void initiateRotation(Minecraft mc, Vec3 targetPos, long minDuration, float humanizeRange) {
        ...
        long configDuration = (long) ConfigHelpers.getRandomizedDelay(TfmConfig.ROTATION_TIME.get());
    ...
    public static void update(Minecraft mc) {
        ...
        if (isRotating && startRot != null && targetRot != null) {
            ...
            if (applyTrackingNoise) {
                float noiseMin = TfmConfig.ROTATION_TRACKING_NOISE_MIN.get();
                float noiseMax = TfmConfig.ROTATION_TRACKING_NOISE_MAX.get();
    ...
    private static float applyEasing(float t) {
        boolean easeIn  = TfmConfig.ROTATION_EASE_IN.get();
        boolean easeOut = TfmConfig.ROTATION_EASE_OUT.get();

        if (!easeIn && !easeOut) return t;

        float inFactor  = TfmConfig.ROTATION_EASE_IN_FACTOR.get();
        float outFactor = TfmConfig.ROTATION_EASE_OUT_FACTOR.get();
    ...
    private static long computeDynamicDuration(RotationUtils.Rotation start, RotationUtils.Rotation end) {
        float msPerDegree = TfmConfig.ROTATION_DYNAMIC_DURATION_MS_PER_DEGREE.get();
        if (msPerDegree <= 0.0f) {
            return 0L;
        }
