package dev.typicalfarmingmacro.modules;
...
public class GreenhouseManager {
    ...
    public static boolean shouldRunAutoGreenhouse() {
        if (!TfmConfig.AUTO_GREENHOUSE.get() || running) {
            return false;
        }

        if (TfmConfig.GREENHOUSE_PLOTS.get().isEmpty()) {
            return false;
        }
    ...
    public static void harvest(Minecraft mc) {
        ...
        List<String> plots = TfmConfig.GREENHOUSE_PLOTS.get().stream().map(String::trim).filter(s -> !s.isEmpty()).distinct().toList();
        if (plots.isEmpty()) {
    ...
    private static void startNextPlot(Minecraft mc) {
        ...
        String currentPlot = dev.typicalfarmingmacro.util.ClientUtils.getCurrentPlot(mc);
        ...
        MacroWorkerThread.getInstance().submit("GreenhouseHarvest-Plot-" + targetPlot, () -> {
            boolean success = dev.typicalfarmingmacro.util.CommandUtils.plotTp(mc, targetPlot);
            if (success) {
    ...
    private static boolean equipHarvestTool(Minecraft mc) {
        if (TfmConfig.EQUIP_GREENHOUSE_CUSTOM_ITEM.get()) {
            String customItem = TfmConfig.GREENHOUSE_CUSTOM_ITEM.get().trim();
    ...
    public static void debugScanSkulls(Minecraft mc) {
        ...
        if (entries.isEmpty()) {
            ClientUtils.sendMessage(mc,
                    String.format(Locale.US,
                            "\u00A7e" + dev.typicalfarmingmacro.util.TfmLang.localize("No skulls with owner profiles found within %d blocks."),
                            SKULL_SCAN_RADIUS),
                    false);
            return;
        }
        ...
        ClientUtils.sendMessage(mc,
                String.format(Locale.US,
                        "\u00A7e" + dev.typicalfarmingmacro.util.TfmLang.localize("Scanned %d skulls within %d blocks."),
                        entries.size(),
                        SKULL_SCAN_RADIUS),
                false);
        ...
        for (SkullScanEntry entry : entries) {
            String line = String.format(Locale.US,
                    dev.typicalfarmingmacro.util.TfmLang.localize("Skull %s (%s) ID: %s at %.1f, %.1f, %.1f"),
                    entry.name(),
                    entry.sourceLabel(),
                    entry.skinId(),
                    entry.x(),
        ...
        for (Entity entity : mc.level.entitiesForRendering()) {
            ...
            if (profile != null) {
                consumer.accept(new SkullScanEntry(
                                ...
                                armorStand.getZ(),
                                dev.typicalfarmingmacro.util.TfmLang.localize("armor stand")));
            }
        }
                ...
                for (int z = -SKULL_SCAN_RADIUS; z <= SKULL_SCAN_RADIUS; z++) {
                    ...
                    if (profile != null) {
                        consumer.accept(new SkullScanEntry(
                                ...
                                pos.getY() + 0.25,
                                pos.getZ() + 0.5,
                                dev.typicalfarmingmacro.util.TfmLang.localize("skull block")));
                    }
                }
            }
        }
    ...
    private static void processProfile(Minecraft mc, ResolvableProfile profile, double x, double y, double z) {
        ...
        if (TfmConfig.HARVEST_ASHWREATH.get() && skinId.equals(ASHWREATH_SKIN_ID)) {
            shouldHighlight = true;
        }
        if (TfmConfig.HARVEST_TURTELLINI.get() && skinId.equals(TURTELLINI_SKIN_ID)) {
            shouldHighlight = true;
        }
        if (TfmConfig.HARVEST_GLASSCORN.get() && skinId.equals(GLASSCORN_SKIN_ID)) {
            shouldHighlight = true;
        }
