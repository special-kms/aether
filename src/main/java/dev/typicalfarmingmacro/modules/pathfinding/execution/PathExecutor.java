package dev.typicalfarmingmacro.modules.pathfinding.execution;

import dev.typicalfarmingmacro.config.TfmConfig;
import dev.typicalfarmingmacro.modules.pathfinding.Node;
import dev.typicalfarmingmacro.modules.pathfinding.Node.MoveType;
import dev.typicalfarmingmacro.modules.pathfinding.rotation.AngleUtils;
import dev.typicalfarmingmacro.modules.pathfinding.rotation.EasingType;
import dev.typicalfarmingmacro.modules.pathfinding.rotation.Rotation;
import dev.typicalfarmingmacro.modules.pathfinding.rotation.RotationExecutor;
import dev.typicalfarmingmacro.modules.pathfinding.rotation.strategy.TimedEaseStrategy;
...
public final class PathExecutor {
    ...
    public void tick(Minecraft mc) {
        ...
        // Periodic debug
        if (TfmConfig.SHOW_DEBUG.get() && ++debugTick % 40 == 0 && pursuitSegment < path.size()) {
        ...
        if (allowReplan && pathStaleDuration > PATH_REPLAN_HARD_STALE_MS && cooldownPassed) {
            System.out.printf("[Tfm] PathExecutor replan: no path progress, stale=%d ms%n",
                    pathStaleDuration);
            triggerReplan(mc);
            return;
        }
        ...
        if (allowReplan && pathProgressStale && cooldownPassed
            ...
            System.out.printf("[Tfm] PathExecutor replan: no path progress, drift=%.2f blk, stale=%d ms%n",
        ...
        if (allowReplan && progressStale && drifted && cooldownPassed) {
            System.out.printf("[Tfm] PathExecutor replan: drift=%.2f blk, stale=%d ms%n",
        ...
        if (staleDuration > STUCK_TIME_MS * 2 && cooldownPassed) {
            System.out.printf("[Tfm] PathExecutor replan: deadlock, stale=%d ms%n", staleDuration);
            triggerReplan(mc);
            return;
        }
