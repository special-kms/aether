package dev.typicalfarmingmacro.modules.pathfinding;

import dev.typicalfarmingmacro.config.TfmConfig;
import dev.typicalfarmingmacro.modules.pathfinding.debug.PathVisualizer;
import dev.typicalfarmingmacro.modules.pathfinding.etherwarp.EtherwarpHelper;
import dev.typicalfarmingmacro.modules.pathfinding.execution.EtherwarpExecutor;
import dev.typicalfarmingmacro.modules.pathfinding.execution.FlyExecutor;
import dev.typicalfarmingmacro.modules.pathfinding.execution.PathExecutor;
import dev.typicalfarmingmacro.modules.pathfinding.movement.PathSmoother;
import dev.typicalfarmingmacro.modules.pathfinding.movement.WalkabilityChecker;
import dev.typicalfarmingmacro.modules.pathfinding.pathfinder.AStarPathfinder;
import dev.typicalfarmingmacro.modules.pathfinding.pathfinder.EtherwarpPathfinder;
import dev.typicalfarmingmacro.modules.pathfinding.pathing.NeighborStrategies;
import dev.typicalfarmingmacro.modules.pathfinding.pathing.configuration.PathfinderConfiguration;
import dev.typicalfarmingmacro.modules.pathfinding.pathing.processing.impl.MinecraftPathProcessor;
...
import dev.typicalfarmingmacro.modules.pathfinding.provider.impl.MinecraftNavigationProvider;
...
public final class PathfindingManager {
    ...
    public static void startPathTest(Minecraft mc, int x, int y, int z) {
        ...
        if (mc.player != null) {
            dev.typicalfarmingmacro.util.ClientUtils.sendMessage(mc, "\u00A7ePath test: running A* to "
                            + x + ", " + finalY + ", " + z + "...", false);
        }
        ...
        Thread t = new Thread(() -> {
            ...
            PathfinderConfiguration config = PathfinderConfiguration.builder()
                    ...
                    .processors(List.of(new MinecraftPathProcessor(checker,
                            TfmConfig.PATHFINDER_MAX_JUMP_HEIGHT.get())))
            ...
            mc.execute(() -> {
                if (finalResult == null) {
                    if (mc.player != null) {
                        dev.typicalfarmingmacro.util.ClientUtils.sendMessage(mc, "\u00A7cPath test error!", false);
                    }
                    return;
                }
                ...
                if (mc.player != null) {
                    dev.typicalfarmingmacro.util.ClientUtils.sendMessage(mc, "\u00A7ePath test result: "
                                    + typeStr
                                    ...
                                    + " | time: " + elapsedMs + "ms",
                            false);
                    dev.typicalfarmingmacro.util.ClientUtils.sendDebugMessage(mc,
    ...
    public static void startGreenhouseWalk(Minecraft mc, net.minecraft.world.phys.Vec3 target, Runnable onFinished, boolean isFirst) {
        ...
        if (mc.player != null) {
            dev.typicalfarmingmacro.util.ClientUtils.sendMessage(mc, "\u00A77Greenhouse path: direct walk to target.", false);
        }
    }
    ...
    public static void stop(boolean announce) {
        ...
        if (announce && wasNavigating && mc.player != null) {
            dev.typicalfarmingmacro.util.ClientUtils.sendMessage(mc, "\u00A7eNavigation stopped.", false);
        }
    }
    ...
    private static void doStartPathfind(Minecraft mc, int x, int y, int z, boolean fly) {
        ...
        if (mc.player != null) {
            dev.typicalfarmingmacro.util.ClientUtils.sendMessage(mc, "\u00A7eFinding path to "
                            + x + ", " + finalY + ", " + z + "...", false);
        }
        ...
        if (fly) {
            ...
            if (smoothed.isEmpty()) {
                ...
                clearTransientDebugRenderingIfActive();
                if (mc.player != null) {
                    dev.typicalfarmingmacro.util.ClientUtils.sendMessage(mc, "\u00A7cFly path build failed!", false);
                }
                return;
            }
            ...
            if (mc.player != null) {
                dev.typicalfarmingmacro.util.ClientUtils.sendMessage(mc, "\u00A7aFly path: "
    ...
    private static void doStartEtherwarpPathfind(Minecraft mc, int x, int y, int z) {
        ...
        if (target == null) {
            dev.typicalfarmingmacro.util.ClientUtils.sendMessage(mc,
        ...
        if (dev.typicalfarmingmacro.modules.gear.GearManager.findEtherwarpAspectOfTheVoidHotbarSlot(mc) < 0) {
            dev.typicalfarmingmacro.util.ClientUtils.sendMessage(mc,
    ...
    private static void startEtherwarpPathSearch(Minecraft mc, PathPosition target, boolean retry) {
        ...
        if (mc.player != null) {
            ...
            dev.typicalfarmingmacro.util.ClientUtils.sendMessage(mc, message, false);
        }
    ...
    private static void handleEtherwarpResult(Minecraft mc,
        ...
        if (!plan.hasPath()) {
            ...
            clearTransientDebugRenderingIfActive();
            if (mc.player != null) {
                dev.typicalfarmingmacro.util.ClientUtils.sendMessage(mc, "\u00A7cNo etherwarp path found!", false);
            }
            if (etherwarpFailureCallback != null) {
                etherwarpFailureCallback.run();
            }
    ...
    private static void handleWalkResult(Minecraft mc, PathfinderResult result,
        ...
        if (!hasPath || positions.isEmpty()) {
            ...
            clearTransientDebugRenderingIfActive();
            if (mc.player != null) {
                dev.typicalfarmingmacro.util.ClientUtils.sendMessage(mc, "\u00A7cNo path found!", false);
            }
    ...
    private static final dev.typicalfarmingmacro.modules.pathfinding.pathing.heuristic.IHeuristicStrategy ZERO_HEURISTIC =
            new dev.typicalfarmingmacro.modules.pathfinding.pathing.heuristic.LinearHeuristicStrategy();

    private static Node makeNode(int x, int y, int z) {
        PathPosition pos  = new PathPosition(x, y, z);
        Node         node = new Node(pos, pos, pos,
                new dev.typicalfarmingmacro.modules.pathfinding.pathing.heuristic.HeuristicWeights(0, 0, 0, 0),
                ZERO_HEURISTIC,
                0);
