package dev.typicalfarmingmacro.modules.forge;
...
public class ForgeManager {
    ...
    public static void start(Minecraft client) {
        if (isRunning) {
            ClientUtils.sendMessage(client, "§c[Tfm] Forge: already running.", false);
            return;
        }
        isRunning = true;
        MacroWorkerThread.getInstance().submit("ForgeManager", () -> {
            try {
                ...
                e.printStackTrace();
                ClientUtils.sendMessage(client, "§c[Tfm] Forge error: " + e.getMessage(), false);
            } finally {
                isRunning = false;
            }
        });
    ...
    private static void run(Minecraft client) throws InterruptedException {
        ...
        ClientUtils.sendMessage(client, "§e[Tfm] Warping to forge...", false);
        if (!warpToForge(client)) {
            ClientUtils.sendMessage(client, "§c[Tfm] Forge warp timed out. Aborting.", false);
            return;
        }
        ...
        ClientUtils.sendMessage(client, "§e[Tfm] Pathfinding to Forger NPC...", false);
        ...
        if (!walkToCoords(client, walkTarget.getX(), walkTarget.getY(), walkTarget.getZ())) {
            ClientUtils.sendMessage(client, "§c[Tfm] Failed to reach Forger NPC. Aborting.", false);
            return;
        }
        ...
        if (forger == null) {
            ClientUtils.sendMessage(client, "§c[Tfm] Could not find Forger NPC. Aborting.", false);
            return;
        }
        ...
        ClientUtils.sendMessage(client, "§e[Tfm] Opening forge menu...", false);
        if (!interactUntilGui(client, forger, 5000)) {
            ClientUtils.sendMessage(client, "§c[Tfm] Forge GUI did not open. Aborting.", false);
            return;
        }

        ClientUtils.sendMessage(client, "§a[Tfm] Forge menu opened successfully.", false);
