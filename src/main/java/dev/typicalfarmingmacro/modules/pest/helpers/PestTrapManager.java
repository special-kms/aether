...
public class PestTrapManager {
    ...
    public static void start(Minecraft client) {
        ...
        String plot = TfmConfig.PEST_TRAPS_PLOT.get();
    ...
    public static void runSequence(Minecraft client, String plot) throws InterruptedException {
        ...
        while (isRunning && !shouldAbort()) {
            ...
            for (int trapId : fullTraps) {
                ...
                if (TfmConfig.AUTO_MOSQUITO_FOR_PEST_TRAPS.get()) {
    ...
    public static void runRefillSequence(Minecraft client, String plot) throws InterruptedException {
        ...
        String baitMaterial = TfmConfig.PEST_TRAPS_BAIT_MATERIAL.get();
        int baitAmount = Math.max(1, TfmConfig.PEST_TRAPS_BAIT_AMOUNT.get());
        int baitNeeded = emptyTraps.size() * baitAmount;
        ClientUtils.sendDebugMessage(client, "Buying " + baitNeeded + " " + baitMaterial + "...");
        boolean bought = dev.typicalfarmingmacro.util.BazaarUtils.executeBuy(client, baitMaterial, baitNeeded);
    ...
    private static void ensurePetEquippedAfterTrapOpen(Minecraft client) throws InterruptedException {
        if (!TfmConfig.AUTO_PET_AFTER_TRAP_OPEN.get()) {
            return;
        }
        BudgetAutopetManager.equipPetByName(client, TfmConfig.AUTO_PET_AFTER_TRAP_OPEN_PET.get(),
                "pest trap release");
    }
