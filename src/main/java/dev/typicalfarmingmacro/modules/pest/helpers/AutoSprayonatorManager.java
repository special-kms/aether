...
public final class AutoSprayonatorManager {
    ...
    public static void update(Minecraft client) {
        ...
        if (!TfmConfig.AUTO_SPRAYONATOR.get()) return;
        if (running) return;
        ...
        if (now - sprayNeededSinceMs < TfmConfig.AUTO_SPRAYONATOR_DETECT_TIME.get() * 1000L) return;
    ...
    public static boolean sprayHeldMaterialAndHandleMissing(Minecraft client, long guiDelay) {
        ...
        if (!TfmConfig.AUTO_SPRAYONATOR_AUTO_BUY.get()) {
        ...
        int amount = Math.max(1, TfmConfig.AUTO_SPRAYONATOR_AUTO_BUY_AMOUNT.get());
