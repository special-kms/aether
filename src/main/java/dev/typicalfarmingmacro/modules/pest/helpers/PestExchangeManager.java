...
import dev.typicalfarmingmacro.util.EntityUtils;
import dev.typicalfarmingmacro.util.RotationUtils;
...
public class PestExchangeManager {
    ...
    public static boolean runExchangeBlocking(Minecraft client) {
        if (isExchanging) {
            dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§cPest exchange is already running.", false);
            return false;
        }
        ...
        dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§eStarting pest exchange...", false);
        ...
        try {
            ...
            client.execute(() -> {
                if (client.player != null)
                    dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§cPest exchange error: " + e.getMessage(), false);
            });
            e.printStackTrace();
            return false;
        } finally {
    ...
    private static boolean runExchange(Minecraft client) {
        ...
        client.execute(() -> {
            if (client.player != null)
                dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§eTeleporting to barn...", false);
        });

        Vec3 posBefore = client.player.position();
        dev.typicalfarmingmacro.modules.failsafe.FailsafeManager.addRotationGracePeriod(dev.typicalfarmingmacro.config.TfmConfig.FAILSAFE_ROTATION_WARP_GRACE_MS.get());
        ...
        int deskX = TfmConfig.PEST_EXCHANGE_DESK_X.get();
        int deskY = TfmConfig.PEST_EXCHANGE_DESK_Y.get();
        int deskZ = TfmConfig.PEST_EXCHANGE_DESK_Z.get();

        ClientUtils.sendDebugMessage(client, "[PestExchange] Walking to desk at " + deskX + ", " + deskY + ", " + deskZ);
        client.execute(() -> {
                if (client.player != null)
                    dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§eWalking to Phillip's desk...", false);
        });
        ...
        if (PathfindingManager.isNavigating()) {
            ...
            client.execute(() -> {
                if (client.player != null)
                    dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§cPathfinding timed out. Stopping pest exchange.", false);
            });
            isExchanging = false;
            return false;
        }
        ...
        if (phillipEntity == null) {
            client.execute(() -> {
                if (client.player != null)
                    dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§ePhillip not found, retrying in 3s...", false);
            });
        ...
        if (phillipEntity == null) {
            client.execute(() -> {
                if (client.player != null)
                    dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§cCould not find Phillip NPC after retry. Stopping.", false);
            });
            isExchanging = false;
            return false;
        }
        ...
        if (!(client.screen instanceof AbstractContainerScreen)) {
            client.execute(() -> {
                if (client.player != null)
                    dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§cFailed to open Phillip's GUI. Stopping.", false);
            });
            isExchanging = false;
            return false;
        }
        ...
        client.execute(() -> {
            ...
            if (vacuumSlot == -1) {
                    if (client.player != null)
                    dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§cCould not find 'Empty Vacuum Bag' slot. Closing.", false);
            ...
            if (lore.contains("Click to empty")) {
                if (client.player != null)
                    dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§aEmptying vacuum bag!", false);
                dev.typicalfarmingmacro.util.ClientUtils.performSlotClick(client, screen, vacuumSlot, 0, ContainerInput.PICKUP);
            } else if (lore.contains("exchanged enough Pests")) {
                if (client.player != null)
                    dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§eAlready emptied the vacuum recently!", false);
                client.player.closeContainer();
            } else {
                if (client.player != null)
                    dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§cVacuum bag state unknown. Closing.", false);
                client.player.closeContainer();
            }
        });
        ...
        client.execute(() -> {
            if (client.player != null)
                dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "§aPest exchange complete!", false);
        });
    ...
    private static boolean runAbiphoneExchange(Minecraft client) {
        for (int attempt = 1; attempt <= MAX_ABIPHONE_CALL_ATTEMPTS && isExchanging; attempt++) {
            ...
            client.execute(() -> dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client,
                    "§eCalling Phillip via Abiphone (" + currentAttempt + "/" + MAX_ABIPHONE_CALL_ATTEMPTS + ")...",
                    false));
            dev.typicalfarmingmacro.util.ClientUtils.sendCommand(client, "/call phillip");
    ...
    private static void facePhillipForInteraction(Minecraft client, Entity phillip) {
        ...
        if (!isLookingAt(client, phillip, TfmConfig.PEST_EXCHANGE_FOV_RANGE.get())) {
    ...
    private static void rotateToPhillip(Minecraft client, Entity phillip) {
        client.execute(() -> RotationManager.initiateRotation(client,
                ...
                TfmConfig.ROTATION_TIME.get(),
                TfmConfig.PEST_EXCHANGE_FOV_RANGE.get()));
        MacroWorkerThread.sleep(TfmConfig.ROTATION_TIME.get() + 50L);
    }
