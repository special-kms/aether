...
final class PestCombatCoordinator {
    ...
    static void handleFlyToPest(
        ...
        if (dist <= targetReachDistance * 1.5
            ...
            if (!context.isLookingAt(client, targetEye, TfmConfig.PEST_FOV_RANGE.get())) {
                RotationManager.initiateRotation(client, targetEye, 80, TfmConfig.PEST_FOV_RANGE.get());
            }
        }
    ...
    static void handleAotvBetweenPests(
        ...
        if (!facingAim) {
            if (!suppressRotation && !RotationManager.isRotating()) {
                RotationManager.initiateRotation(client, aimPos, TfmConfig.ROTATION_TIME.get());
            }
            if (!suppressRotation) {
                return;
            }
        ...
        if (TfmConfig.PEST_AOTV_CONFIRM_BETWEEN.get() && context.getAotvPendingUseAt() != 0L) {
        ...
        if (readyAt == 0L) {
            ...
            readyAt = anchor + dev.typicalfarmingmacro.config.ConfigHelpers.getRandomizedDelay(
        ...
        if (now >= readyAt) {
            ...
            if (TfmConfig.PEST_AOTV_CONFIRM_BETWEEN.get()) {
    ...
    private static float getAbovePestPitch(Entity target) {
        float minPitch = TfmConfig.PEST_ABOVE_TARGET_PITCH_MIN.get();
        float maxPitch = TfmConfig.PEST_ABOVE_TARGET_PITCH_MAX.get();
