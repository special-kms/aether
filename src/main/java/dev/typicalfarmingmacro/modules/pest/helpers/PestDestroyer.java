...
import dev.typicalfarmingmacro.util.CommandUtils;
...
public class PestDestroyer {
    ...
    public static void start(Minecraft client, String initialPlot) {
        ...
        dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "\u00A7ePest destroyer active. Hunting pests...", false);
    ...
    public static void update(Minecraft client) {
        ...
        if (runtime.state == State.AOTV_TO_ROOF) {
            ...
            if (!Double.isNaN(runtime.aotvStartY)) {
                ...
                int aotvHotbarSlot = dev.typicalfarmingmacro.modules.gear.GearManager.findAspectOfTheVoidSlot(client);
                if (aotvHotbarSlot != -1 && ((dev.typicalfarmingmacro.mixin.AccessorInventory) client.player.getInventory()).getSelected() == aotvHotbarSlot) {
        ...
        if (System.currentTimeMillis() - runtime.activatedAt > STUCK_TIMEOUT_MS) {
            dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "\u00A7cPest destroyer timed out after 5 minutes. Returning to farm.", false);
            finish(client);
            return;
        }
    ...
    private static boolean tryStartPeriodicRoofAotv(Minecraft client) {
        if (!TfmConfig.AOTV_TO_ROOF.get() || !isPeriodicRoofRescanState(runtime.state)) {
            return false;
        }
    ...
    private static boolean shouldAotvToRoofOnPlot(String plot) {
        ...
        return plot != null && TfmConfig.AOTV_ROOF_PLOTS.get().contains(plot);
    }
    ...
    private static void handleEquipVacuum(Minecraft client) {
        ...
        if (slot == -1) {
            dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "\u00A7cNo vacuum found in hotbar. Aborting pest destroyer.", false);
            finish(client);
            return;
        }
        runtime.vacuumSlot = slot;
        ...
        if (!client.player.getAbilities().flying && client.player.getAbilities().mayfly) {
            ...
            dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "\u00A7cCannot fly. Aborting pest destroyer.", false);
            finish(client);
        }
    }
    ...
    private static void rotateToTarget(Minecraft client, Entity target) {
        ...
        if (!isLookingAt(client, targetEye, TfmConfig.PEST_FOV_RANGE.get())) {
            RotationManager.initiateRotation(client, targetEye, TARGET_SWITCH_ROTATION_MS, TfmConfig.PEST_FOV_RANGE.get());
        }
    }

    private static boolean isLookingAt(Minecraft client, Vec3 targetPos, float tolerance) {
        if (client.player == null) return false;
        return dev.typicalfarmingmacro.util.RotationUtils.isLookingAt(
    ...
    public static void finish(Minecraft client) {
        ...
        dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "\u00A7aPest destroyer finished. Tracked " + killed + " pest(s).", false);
        runtime.active = false;
        runtime.state = State.IDLE;
        runtime.currentTarget = null;
        runtime.killedEntities.clear();
    ...
    public static boolean shouldFinishForAliveCount(Minecraft client, int aliveCount) {
        ...
        if (!TfmConfig.LEAVE_ONE_PEST_ALIVE.get()) {
            return false;
        }
    ...
    private static boolean isLeaveOneWhitelistedPlot(String plot) {
        ...
        for (String configuredPlot : TfmConfig.LEAVE_ONE_PEST_PLOTS.get()) {
    ...
    private static void engagePestTarget(Minecraft client, Entity pest) {
        ...
        if (dist <= runtime.vacuumRange) {
            ...
            setState(State.KILL_PEST);
        } else if (dist > AOTV_RANGE * AOTV_GAP_MULTIPLIER && runtime.aotvSlot != -1 && TfmConfig.PEST_AOTV_BETWEEN.get()) {
