...
public final class VacuumParticleDebug {
    private static final Logger LOGGER = LoggerFactory.getLogger("typicalfarmingmacro");
    ...
    public static void onParticlePacket(Minecraft client, ClientboundLevelParticlesPacket packet) {
        if (!TfmConfig.SHOW_DEBUG.get() || packet == null) {
            return;
        }
    ...
    private static boolean isVacuumLeftClickActive(Minecraft client) {
        ...
        int selected = ((dev.typicalfarmingmacro.mixin.AccessorInventory) client.player.getInventory()).getSelected();
