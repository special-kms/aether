package dev.typicalfarmingmacro.modules.pest;
...
import dev.typicalfarmingmacro.modules.pest.helpers.PestCleaningSequencer;
import dev.typicalfarmingmacro.modules.pest.helpers.PestDiscoDestinationManager;
...
public class PestManager {
    ...
    private static boolean isThresholdMet(int aliveCount) {
        return aliveCount >= TfmConfig.PEST_THRESHOLD.get() || aliveCount >= 8;
    }

    public static boolean isPestDestroyerEnabled() {
        return TfmConfig.TRIGGER_PEST_ON_CHAT.get();
    }

    public static boolean arePestTrapsEnabled() {
        return TfmConfig.ENABLE_PEST_TRAPS.get();
    }
    ...
    private static boolean canTriggerAfterRewarp() {
        return !TfmConfig.PEST_TRIGGER_ONLY_AFTER_REWARP.get() || rewarpTriggerAvailable;
    }

    private static void consumeRewarpTrigger() {
        if (TfmConfig.PEST_TRIGGER_ONLY_AFTER_REWARP.get()) {
    ...
    public static void checkTabListForPests(Minecraft client, MacroState.State currentState) {
        ...
        if (currentState == MacroState.State.CLEANING && !GreenhouseManager.isRunning()) {
            ...
            if (effectiveAlive <= 0) {
                if (lastZeroPestTime == 0) {
                    ...
                    dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "\u00A7cFail-safe: No pests detected for 10s. Returning to farm.", true);
                    lastZeroPestTime = 0;
        ...
        if (dev.typicalfarmingmacro.modules.visitor.VisitorsMacro.isRunning) {
            return;
        }
        ...
        if (isThresholdMet(effectiveAlive)) {
            ...
            if (TfmConfig.DELAY_PEST_FOR_CROP_FEVER.get() && dev.typicalfarmingmacro.modules.CropFeverManager.isCropFeverActive) {
                return;
            }

            if (effectiveAlive >= 8 && effectiveAlive < 99) {
                dev.typicalfarmingmacro.util.ClientUtils.sendMessage(client, "\u00A7eMax Pests (8) reached. Starting cleaning...", true);
            }
            ...
            if (TfmConfig.SHOW_DEBUG.get()) {
                ClientUtils.sendDebugMessage(client,
                        "Chat pest trigger ignored: effective=" + effectiveAlive
                                ...
                                + ") < threshold=" + TfmConfig.PEST_THRESHOLD.get());
            }
            return false;
        }
        ...
        if (predictedAliveCount > 0) {
            ...
            ClientUtils.sendDebugMessage(client,
                    "Pest kill detected! Predicted alive: " + predictedAliveCount
                            ...
                            + ", whitelistedPlots=" + TfmConfig.LEAVE_ONE_PEST_PLOTS.get());
