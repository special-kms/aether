...
/**
 * Standalone visitors macro triggered by {@code /typicalfarmingmacro visitors}.
 * <p>
 * Flow:
 * <ol>
...
public class VisitorsMacro {
    ...
    public static synchronized void start(Minecraft client, boolean forceNoResume) {
        ...
        wasRunningBefore = !forceNoResume && dev.typicalfarmingmacro.macro.MacroStateManager.isMacroRunning();
        isRunning = true;
        shouldStop = false;
        ...
        MacroWorkerThread.getInstance().submit("VisitorsMacro", () -> {
            ...
            try {
                ...
                if (wasRunningBefore) {
                    dev.typicalfarmingmacro.macro.MacroStateManager.setCurrentState(dev.typicalfarmingmacro.macro.MacroState.State.VISITING);
                }
    ...
    private static void run(Minecraft client) {
        ...
        dev.typicalfarmingmacro.modules.inventorymanager.AutoSellManager.checkBeforeVisitors(client, true, wasRunningBefore);
        ...
        FailsafeManager.addRotationGracePeriod(dev.typicalfarmingmacro.config.TfmConfig.FAILSAFE_ROTATION_WARP_GRACE_MS.get());
        ...
        if (TfmConfig.DISABLE_COMPACTORS_DURING_VISITORS.get()) {
        ...
        if (!shouldStop && wasRunningBefore) {
            ...
            dev.typicalfarmingmacro.macro.MacroStateManager.setCurrentState(dev.typicalfarmingmacro.macro.MacroState.State.OFF);
        }
        } finally {
    ...
    private static boolean isPurchaseWithinLimit(Minecraft client, ItemRequirement req) {
        int limit = TfmConfig.VISITOR_MAX_PURCHASE_LIMIT.get();
    ...
    private static List<String> filterignoredVisitors(Minecraft client, List<String> visitorNames) {
        ...
        List<String> ignore = TfmConfig.VISITOR_ignore.get();
    ...
    private static boolean isRejectedVisitor(String name) {
        ...
        for (String rej : TfmConfig.VISITOR_REJECT.get()) {
    ...
    private static void faceVisitorForInteraction(Minecraft client, Entity entity) {
        ...
        if (!isLookingAt(client, entity, TfmConfig.VISITOR_FOV_RANGE.get())) {
            rotateToEntity(client, entity);
            waitForRotationToFinish();
        }
    }
    ...
    private static boolean interactWithEntity(Minecraft client, Entity entity) {
        ...
        client.execute(() -> {
            ...
            ((MixinMinecraft) client).typicalfarmingmacro$startAttack();
        });
    ...
    private static boolean interactUntilGui(Minecraft client, String visitorName, Entity entity, long timeoutMs) {
        ...
        while (System.currentTimeMillis() < deadline && !shouldStop) {
            ...
            if (!isVisitorInRetryRange(client, entity, retryRange)
                    || !isLookingAt(client, entity, TfmConfig.VISITOR_FOV_RANGE.get())) {
    ...
    private static boolean waitForTeleport(Minecraft client, Vec3 posBefore, CommandUtils.ChatWindow window, long timeoutMs) {
        ...
        while (System.currentTimeMillis() < deadline && !shouldStop) {
            if (hasBarnTeleportMessage)
                return true;
            if (dev.typicalfarmingmacro.util.CommandUtils
    ...
    private static boolean equipVisitorTool(Minecraft client) {
        if (TfmConfig.EQUIP_VISITOR_CUSTOM_ITEM.get()) {
            String customItem = TfmConfig.VISITOR_CUSTOM_ITEM.get().trim();
