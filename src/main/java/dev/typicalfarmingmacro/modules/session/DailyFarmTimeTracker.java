...
public final class DailyFarmTimeTracker {
    ...
    public static void syncFromConfig() {
        String today = today();
        String savedDate = TfmConfig.DAILY_FARM_DATE.get();
        if (today.equals(savedDate)) {
            accumulatedMs = Math.max(0L, (long) (double) TfmConfig.DAILY_FARM_ACCUMULATED.get());
        } else {
            accumulatedMs = 0L;
            TfmConfig.DAILY_FARM_DATE.set(today);
            TfmConfig.DAILY_FARM_ACCUMULATED.set(0.0);
            TfmConfig.save();
        }
        trackedDate = today;
    ...
    private static void persist(boolean flush) {
        TfmConfig.DAILY_FARM_DATE.set(trackedDate);
        TfmConfig.DAILY_FARM_ACCUMULATED.set((double) getCurrentAccumulatedMs());
        if (flush) {
            TfmConfig.save();
        }
    }
