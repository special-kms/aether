package dev.typicalfarmingmacro.modules.session;
...
import dev.typicalfarmingmacro.ui.DynamicRestScreen;
import dev.typicalfarmingmacro.ui.FunnyDynamicRestScreen;
...
public class DynamicRestManager {
    ...
    private static boolean isEnabled() {
        return TfmConfig.DYNAMIC_REST_ENABLED.get();
    }
    ...
    public static void scheduleNextRest() {
        ...
        if (TfmConfig.PERSIST_SESSION_TIMER.get() && nextRestTriggerMs != 0) {
            return;
        }
    ...
    private static RestWindow createRestWindow(long nowMs, long elapsedMs) {
        long baseMs = TfmConfig.REST_SCRIPTING_TIME.get() * 60L * 1000L;
        long offsetMs = TfmConfig.REST_SCRIPTING_TIME_OFFSET.get() * 60L * 1000L;
    ...
    public static void update(Minecraft client) {
        ...
        switch (restSequenceStage) {
            case 0: {
                ...
                dev.typicalfarmingmacro.util.CommandUtils.initiateSetSpawn(client);
            ...
            case 1: {
                if (!dev.typicalfarmingmacro.util.CommandUtils.hasSpawnBeenSet(restSetSpawnWindow)
                        && System.currentTimeMillis() < nextStageActionTime) {
                    return;
                }

                long baseSecs = TfmConfig.REST_BREAK_TIME.get() * 60L;
                long offsetSecs = TfmConfig.REST_BREAK_TIME_OFFSET.get() * 60L;
                ...
                Screen restScreen = TfmConfig.FUNNY_DYNAMIC_REST.get()
    ...
    private static void checkDailyThreshold(Minecraft client) {
        double thresholdHours = TfmConfig.DAILY_FARM_THRESHOLD_HOURS.get();
        ...
        ClientUtils.sendMessage(client, "\u00A7e" + String.format(
                TfmLang.localize("Daily farming threshold reached (%.2f hours). Stopping macro..."),
                thresholdHours), false);
        MacroStateManager.stopMacro(client, "Dynamic Rest: daily farming threshold reached");
        ReconnectScheduler.cancel();

        if (TfmConfig.CLOSE_GAME_ON_DAILY_THRESHOLD.get()) {
