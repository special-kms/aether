package dev.typicalfarmingmacro.modules.farming;

import dev.typicalfarmingmacro.config.TfmConfig;
import dev.typicalfarmingmacro.macro.AbstractMacro;
...
public final class FastLaneSwitchManager {
    ...
    public static boolean shouldFastSwitch(Minecraft mc, AbstractMacro.State state) {
        if (!TfmConfig.MACRO_FAST_LANE_SWITCH.get() || mc == null || mc.player == null || !isRowState(state)) {
            return false;
        }
    ...
    public static String getDisplayText() {
        ...
        if (!TfmConfig.MACRO_FAST_LANE_SWITCH.get()
                || macro == null
                || mc.player == null
    ...
    public static boolean hasEstimate() {
        AbstractMacro macro = FarmingMacroManager.getActiveMacro();
        Minecraft mc = Minecraft.getInstance();
        if (!TfmConfig.MACRO_FAST_LANE_SWITCH.get() || macro == null || mc.player == null) {
            return false;
        }
    ...
    public static boolean hasVisibleHighlights() {
        Minecraft mc = Minecraft.getInstance();
        return TfmConfig.MACRO_FAST_LANE_SWITCH.get()
                && mc.level != null
                && mc.player != null
    ...
    public static void renderWorld() {
        ...
        int minBoundary = Math.min(TfmConfig.MACRO_FAST_LANE_LEFT_BOUNDARY.get(),
                TfmConfig.MACRO_FAST_LANE_RIGHT_BOUNDARY.get());
        int maxBoundary = Math.max(TfmConfig.MACRO_FAST_LANE_LEFT_BOUNDARY.get(),
                TfmConfig.MACRO_FAST_LANE_RIGHT_BOUNDARY.get());
    ...
    private static void renderBoundary(PlotBounds plot, int boundary, int strokeColor, int fillColor) {
        ...
        AABB box = "X".equalsIgnoreCase(TfmConfig.MACRO_FAST_LANE_BOUNDARY_AXIS.get())
                ? new AABB(
    ...
    private static double axisDirection(Minecraft mc, AbstractMacro.State state) {
        ...
        return "X".equalsIgnoreCase(TfmConfig.MACRO_FAST_LANE_BOUNDARY_AXIS.get()) ? x : z;
    }

    private static double currentAxisCoord(Minecraft mc) {
        return "X".equalsIgnoreCase(TfmConfig.MACRO_FAST_LANE_BOUNDARY_AXIS.get())
                ? mc.player.getX()
                : mc.player.getZ();
    }
    ...
    private static boolean hasValidBoundaries() {
        return TfmConfig.MACRO_FAST_LANE_LEFT_BOUNDARY.get()
                != TfmConfig.MACRO_FAST_LANE_RIGHT_BOUNDARY.get();
    }
